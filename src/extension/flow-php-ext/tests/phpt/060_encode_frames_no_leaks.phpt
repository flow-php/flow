--TEST--
repeated fused encodeFrames, refusals included, does not leak memory
--SKIPIF--
...
// the hydrated batch schema carries id's folded metadata, which the session schema does not - VALUE_*_WITH_META frames
$rows = (new PhpRowHydrator())->hydrate($batch, $schema);
$refused = Flow\ETL\Rows::trusted($rows->schema(), [...$rows->all(), row(['id' => 101, 'name' => 'x'])]);

$cycle = static function () use ($rows, $refused, $schemaBody, $schema): void {
    $native = new RustFloeEncoderNative();
    $native->encodeFrames($rows, $schemaBody, $schema);

    try {
        $native->encodeFrames($refused, $schemaBody, $schema);
