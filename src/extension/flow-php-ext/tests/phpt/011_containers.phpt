...
use function Flow\Types\DSL\{type_list, type_map, type_structure, structure_element, type_integer, type_float, type_string, type_optional};
...
$rows = rows(
    row(
        ...
        structure_entry('optional_absent', ['a' => 1], type_structure(['a' => type_integer(), 'b' => structure_element('b', type_string(), optional: true)])),
        structure_entry('optional_typed_null', ['a' => 1, 'b' => null], type_structure(['a' => type_integer(), 'b' => type_optional(type_string())])),
        structure_entry('interleaved', ['z' => 1, 'b' => 'x'], type_structure(['z' => type_integer(), 'a' => structure_element('a', type_string(), optional: true), 'b' => type_string()])),
    ),
);
...
var_dump($actual[0]->get('interleaved')->value());
?>
--EXPECT--
identical
array(3) {
