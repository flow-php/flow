...
trait CSVOptions
{
    private function addCSVInputOptions(Command $command): void
    {
        $command
            ->addOption(
                ...
                'When set, CSV header will be used as a schema',
            )
            ->addOption(
                'input-csv-empty-to-null',
                null,
                InputOption::VALUE_OPTIONAL,
                'When set, empty CSV values will be treated as NULL values',
            )
    ...
    private function addCSVOutputOptions(Command $command): void
    {
        $command
            ...
            ->addOption(
                'output-csv-date-time-format',
                null,
                InputOption::VALUE_REQUIRED,
                'DateTime format for CSV output',
            );
    }
}
