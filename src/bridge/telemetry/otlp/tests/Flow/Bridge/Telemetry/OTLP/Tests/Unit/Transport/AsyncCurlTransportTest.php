...
final class AsyncCurlTransportTest extends TestCase
{
    ...
    public function test_creates_transport_via_dsl_function(): void
    {
        ...
        $transport = otlp_async_curl_transport('http://localhost:4318', otlp_json_serializer());

        static::assertInstanceOf(AsyncCurlTransport::class, $transport);
        $transport->shutdown();
    }
    ...
    public function test_creates_transport_with_custom_headers(): void
    {
        ...
        $transport = otlp_async_curl_transport(
            'http://localhost:4318',
            otlp_json_serializer(),
            otlp_async_curl_options()->withHeader('Authorization', 'Bearer token')->withHeader('X-Custom', 'value'),
        );
    ...
    public function test_creates_transport_with_custom_timeouts(): void
    {
        ...
        $transport = otlp_async_curl_transport(
            'http://localhost:4318',
            otlp_json_serializer(),
            otlp_async_curl_options()->withTimeout(2000)->withConnectTimeout(500),
        );
    ...
    public function test_creates_transport_with_options_object(): void
    {
        ...
        $transport = new AsyncCurlTransport('http://localhost:4318', new JsonSerializer(), $options);
    ...
    public function test_shutdown_surfaces_curl_connection_failures_to_error_handler(): void
    {
        ...
        $transport = otlp_async_curl_transport(
            'http://127.0.0.1:1',
            otlp_json_serializer(),
            otlp_async_curl_options()->withConnectTimeout(1)->withTimeout(1),
    ...
    public function test_tick_is_noop_when_no_pending_requests(): void
    {
        ...
        $transport = new AsyncCurlTransport('http://localhost:4318', new JsonSerializer());
