 ...
 * subsequent send() calls, on tick(), or on shutdown().
 */
final class AsyncCurlTransport implements Transport
{
    ...
    public function __construct(
        ...
        private readonly AsyncCurlTransportOptions $options = new AsyncCurlTransportOptions(),
        private readonly ?Transport $failover = null,
        private readonly ErrorHandler $errorHandler = new ErrorLogHandler(),
    ) {
        if (!extension_loaded('curl')) {
            throw new RuntimeException('ext-curl is required for AsyncCurlTransport');
        }
    ...
    public function tick(): void
    {
        ...
        $this->waitForCompletion(microtime(true) + ($this->options->pumpTimeoutMs() / 1000));
