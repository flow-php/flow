...
final class SFTPDataFrameTest extends SFTPTestCase
{
    ...
    {
        $config = config_builder()->mount($this->sftpContext()->filesystem());

        $report = data_frame($config)
            ->read(new FakeStaticOrdersExtractor(1_000))
            ->drop('address', 'notes', 'items')
            ->write(to_csv(path('sftp:///upload/orders.csv')))
            ->run(analyze: true);

        static::assertSame(1_000, $report->statistics()->totalRows());

        $rows = data_frame($config)->read(from_csv(path('sftp:///upload/orders.csv')))->fetch();

        static::assertCount(1_000, $rows);
        static::assertSame('user-0@example.com', $rows->first()->valueOf('email'));
        static::assertSame('user-999@example.com', $rows->last()?->valueOf('email'));
    }
    ...
    public function test_orders_written_as_parquet_survive_a_round_trip_with_every_entry_type(): void
    {
        ...
        $report = data_frame($config)
            ->read(new FakeStaticOrdersExtractor(1_000))
            ->write(to_parquet(path('sftp:///upload/orders.parquet')))
            ->run(analyze: true);

        static::assertSame(1_000, $report->statistics()->totalRows());

        $rows = data_frame($config)->read(from_parquet(path('sftp:///upload/orders.parquet')))->fetch();

        static::assertCount(1_000, $rows);

        $first = $rows->first();

        static::assertSame(0, $first->valueOf('index'));
        static::assertSame(
            '254d61c5-22c8-4407-83a2-76f1cab53af2',
            type_uuid()->assert($first->valueOf('order_id'))->toString(),
        );
        static::assertSame(
            '2025-01-01 12:00:00',
            type_datetime()->assert($first->valueOf('created_at'))->format('Y-m-d H:i:s'),
        );
        static::assertSame(
            ['street' => '123 Main St, Apt 0', 'city' => 'City ', 'zip' => '12345-0', 'country' => 'PL'],
            $first->valueOf('address'),
        );
        static::assertSame(
    ...
    public function test_reading_every_csv_matching_a_recursive_pattern(): void
    {
        ...
        data_frame($config)
            ...
            ->drop('address', 'notes', 'items')
            ->write(to_csv(path('sftp:///upload/2024/01/orders.csv')))
            ->run();

        data_frame($config)
            ->read(new FakeStaticOrdersExtractor(100))
            ->drop('address', 'notes', 'items')
            ->write(to_csv(path('sftp:///upload/2024/02/orders.csv')))
            ->run();

        static::assertCount(200, data_frame($config)->read(from_csv(path('sftp:///upload/**/*.csv')))->fetch());
    }

    public function test_a_dataset_larger_than_a_single_block_is_uploaded_in_parts(): void
    {
        $config = config_builder()->mount($this->sftpContext()->filesystem(sftp_filesystem_options()->withBlockSize(256
        * 1024)));
        ...
        static::assertCount(10_000, data_frame($config)->read(from_csv(path('sftp:///upload/orders.csv')))->fetch());
    }
}
