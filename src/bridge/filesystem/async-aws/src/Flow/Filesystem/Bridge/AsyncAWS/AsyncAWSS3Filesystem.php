...
final readonly class AsyncAWSS3Filesystem implements Filesystem
{
    ...
    public function list(Path $path, Filter $pathFilter = new KeepAll()) : \Generator
    {
        ...
        do {
            ...
            foreach ($result->getContents() as $object) {
                $objectPath = Path::from($path->protocol()->scheme() . DIRECTORY_SEPARATOR . \ltrim((string) $object->getKey(), DIRECTORY_SEPARATOR), $path->options());
    ...
    public function rm(Path $path) : bool
    {
        ...
        try {
             ...
             */
            $folderPath = Path::from(\trim($path->uri(), DIRECTORY_SEPARATOR) . DIRECTORY_SEPARATOR, $path->options());
    ...
    public function status(Path $path) : ?FileStatus
    {
        ...
        if (!$path->isPattern()) {
            ...
            try {
                 ...
                 */
                $folderPath = Path::from(trim($path->uri(), DIRECTORY_SEPARATOR) . DIRECTORY_SEPARATOR, $path->options());
