...
final class MemoryFilesystemFactoryTest extends TestCase
{
    ...
    {
        self::assertSame('memory', (new MemoryFilesystemFactory())->create('memory', [])->mount()->protocol);
    }

    public function test_throws_on_unknown_options() : void
    {
        $this->expectException(InvalidArgumentException::class);
        $this->expectExceptionMessage('Filesystem factory for type "memory" does not accept any options. Unknown keys: [foo, bar].');
    ...
    {
        self::assertSame('memory', (new MemoryFilesystemFactory())->type());
    }
}
