...
final class TelemetryFactory
{
    public static function create(Configuration $config) : Telemetry
    {
        ...
        $otlpExporter = otlp_exporter($transport);
        $voidExporter = void_exporter();

        $spanProcessor = $config->emitTraces
            ? pass_through_span_processor($otlpExporter)
            : pass_through_span_processor($voidExporter);

        $metricProcessor = $config->emitMetrics
            ? pass_through_metric_processor($otlpExporter)
            : pass_through_metric_processor($voidExporter);

        $logProcessor = pass_through_log_processor($voidExporter);
