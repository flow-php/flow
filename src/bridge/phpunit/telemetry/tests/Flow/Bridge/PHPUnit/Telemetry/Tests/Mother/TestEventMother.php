...
use PHPUnit\Event\Test\{Finished, PreparationStarted};
...
final class TestEventMother
{
    ...
    public static function finished(string $className = self::class, string $methodName = 'test_example') : Finished
    {
        return new Finished(
            self::createInfo(),
            self::createTestMethod($className, $methodName),
            1,
        );
    }
     ...
     * @param non-empty-string $methodName
     */
    public static function preparationStarted(string $className = self::class, string $methodName = 'test_example') : PreparationStarted
    {
