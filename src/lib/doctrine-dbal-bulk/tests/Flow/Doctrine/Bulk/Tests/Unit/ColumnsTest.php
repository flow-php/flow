...
final class ColumnsTest extends TestCase
{
    public function test_adds_index_on_the_end_of_all_columns(): void
    {
        ...
        static::assertEquals(new Columns('date_7', 'title_7', 'description_7', 'quantity_7'), $columns->suffix('_7'));
    }
    ...
    public function test_that_collection_contains_columns(): void
    {
        ...
        static::assertTrue($columns->has('date'));
        static::assertTrue($columns->has('date', 'title'));
        static::assertFalse($columns->has('row'));
    }

    public function test_that_collection_not_contains_columns(): void
    {
        $columns = new Columns('date', 'title', 'description', 'quantity');
        static::assertEquals(new Columns('title', 'description', 'quantity'), $columns->without('date'));

        $columns = new Columns('date', 'title', 'description', 'quantity');
        static::assertEquals(new Columns('title', 'quantity'), $columns->without('date', 'description'));
    }
