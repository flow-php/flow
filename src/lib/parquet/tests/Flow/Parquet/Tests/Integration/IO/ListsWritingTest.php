...
final class ListsWritingTest extends TestCase
{
    ...
    {
        $path = __DIR__ . '/var/test-writer-parquet-test-' . \Flow\ETL\NativePHPRandomValueGenerator::string(32) . '.parquet';
    ...
    public function test_writing_list_of_ints() : void
    {
        ...
        $inputData = \array_merge(...\array_map(static function (int $i) use ($faker) : array {
            return [
                [
                    'list_of_ints' => \array_map(static fn ($i) => $faker->numberBetween(0, Consts::PHP_INT32_MAX), \range(1, \Flow\ETL\NativePHPRandomValueGenerator::int(2, 10))),
                ],
            ];
        }, \range(1, 100)));
    ...
    public function test_writing_list_of_strings() : void
    {
        ...
        $inputData = \array_merge(...\array_map(static function (int $i) use ($faker) : array {
            return [
                [
                    'list_of_strings' => \array_map(static fn ($i) => $faker->text(10), \range(1, \Flow\ETL\NativePHPRandomValueGenerator::int(2, 10))),
                ],
            ];
        }, \range(1, 100)));
            ...
            return [
                [
                    'list_of_structs' => \array_map(static fn ($i) => [
                        ...
                        'name' => $faker->text(10),
                    ], \range(1, \Flow\ETL\NativePHPRandomValueGenerator::int(2, 10))),
                ],
            ];
        }, \range(1, 10)));
    ...
    public function test_writing_list_with_nullable_elements() : void
    {
        ...
        $inputData = \array_merge(...\array_map(static function (int $i) use ($faker) : array {
            return [
                [
                    'list_of_ints' => $i % 2 === 0
                        ? \array_map(static fn ($a) => $faker->numberBetween(0, Consts::PHP_INT32_MAX), \range(1, \Flow\ETL\NativePHPRandomValueGenerator::int(2, 10)))
                        : null,
                ],
            ];
        }, \range(1, 100)));
    ...
    public function test_writing_list_with_nullable_list_values() : void
    {
        ...
        $inputData = \array_merge(...\array_map(static function (int $i) use ($faker) : array {
            return [
                [
                    'list_of_ints' => $i % 2 === 0
                        ? \array_map(static fn ($a) => $faker->numberBetween(0, Consts::PHP_INT32_MAX), \range(1, \Flow\ETL\NativePHPRandomValueGenerator::int(2, 2)))
                        : [null, null],
                ],
            ];
        }, \range(1, 10)));
    ...
    public function test_writing_nullable_list_of_ints() : void
    {
        ...
        $inputData = \array_merge(...\array_map(static function (int $i) use ($faker) : array {
            return [
                [
                    'list_of_ints' => $i % 2 === 0
                        ? \array_map(static fn ($i) => $faker->numberBetween(0, Consts::PHP_INT32_MAX), \range(1, \Flow\ETL\NativePHPRandomValueGenerator::int(2, 10)))
                        : null,
                ],
            ];
        }, \range(1, 100)));
