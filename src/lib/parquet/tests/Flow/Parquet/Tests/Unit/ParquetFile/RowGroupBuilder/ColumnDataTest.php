...
final class ColumnDataTest extends TestCase
{
    ...
    public function test_iterating_over_flat_column_data() : void
    {
        ...
        $columnData->add(new FlatValue($column, 0, 1, 1));
        $columnData->add(new FlatValue($column, 0, 1, 2));
        $columnData->add(new FlatValue($column, 0, 1, 3));
    ...
    public function test_iterating_over_map_column_data() : void
    {
        ...
        $columnData = ColumnData::initialize($schema->get('m'));
        $columnData->add(new FlatValue($keyColumn, 0, 2, 'a'));
        $columnData->add(new FlatValue($keyColumn, 2, 2, 'b'));

        $columnData->add(new FlatValue($valueColumn, 0, 3, 1));
        $columnData->add(new FlatValue($valueColumn, 2, 3, 2));
