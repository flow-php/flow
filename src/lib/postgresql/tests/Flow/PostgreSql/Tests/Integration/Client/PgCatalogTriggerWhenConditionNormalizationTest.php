...
final class PgCatalogTriggerWhenConditionNormalizationTest extends PostgreSqlTestCase
{
    ...
    public function test_trigger_when_condition_round_trip_with_implicit_cast() : void
    {
        ...
        $this->pgsqlContext()->client()->execute(
            create()->trigger('trg_orders_when')
        ...
        self::assertSame('trg_orders_when', $dbTrigger->name);
        self::assertNotNull(
            $dbTrigger->whenCondition,
            'whenCondition must be populated from pg_trigger.tgqual',
        );
        self::assertTrue(
