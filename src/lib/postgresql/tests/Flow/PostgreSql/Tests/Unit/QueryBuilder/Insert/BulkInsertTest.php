...
final class BulkInsertTest extends TestCase
{
    ...
    public function test_immutability_on_conflict_do_nothing() : void
    {
        ...
        self::assertStringNotContainsString('ON CONFLICT', $query1->toSql());
        self::assertStringContainsString('ON CONFLICT', $query2->toSql());
    }
    ...
    public function test_immutability_on_conflict_do_update() : void
    {
        ...
        self::assertNotSame($query1, $query2);
        self::assertStringNotContainsString('ON CONFLICT', $query1->toSql());
        self::assertStringContainsString('DO UPDATE', $query2->toSql());
    }
    ...
    public function test_large_batch_placeholder_generation() : void
    {
        ...
        self::assertStringContainsString('$1, $2, $3', $sql);
        self::assertStringContainsString('$298, $299, $300', $sql);
        self::assertStringNotContainsString('$301', $sql);
    }
    ...
    {
        $query = BulkInsert::into('users', ['email', 'name', 'age'], 1)
    ...
    {
        $query = BulkInsert::into('users', ['email', 'name'], 1)
            ->onConflictDoUpdate(ConflictTarget::constraint('users_pkey'), ['name']);
    ...
    {
        $query = BulkInsert::into('users', ['email', 'name', 'age'], 1)
            ->onConflictDoUpdate(ConflictTarget::columns(['email']), ['name', 'age']);
    ...
    {
        $query = BulkInsert::into('orders', ['customer_id', 'product_id', 'quantity'], 1)
            ->onConflictDoUpdate(ConflictTarget::columns(['customer_id', 'product_id']), ['quantity']);
    ...
    public function test_placeholder_offset_calculation() : void
    {
        ...
        self::assertStringContainsString('($1, $2)', $sql);
        self::assertStringContainsString('($3, $4)', $sql);
        self::assertStringContainsString('($5, $6)', $sql);
    }
