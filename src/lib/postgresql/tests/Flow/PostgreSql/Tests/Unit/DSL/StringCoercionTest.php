...
use function Flow\PostgreSql\DSL\{agg_sum, asc, col, delete, eq, literal, select, star, table, update};
...
final class StringCoercionTest extends TestCase
{
    ...
    {
        self::assertSame(
            select(star())->from(table('users'))->orderBy(asc(col('created_at')))->toSql(),
    ...
    {
        self::assertSame(
            select(star())->from(table('users'))->where(\Flow\PostgreSql\DSL\between(col('age'), literal(18), literal(65)))->toSql(),
            select(star())->from('users')->where(\Flow\PostgreSql\DSL\between('age', literal(18), literal(65)))->toSql(),
        );
    }
    ...
    {
        self::assertSame(
            select(\Flow\PostgreSql\DSL\cast(col('id'), \Flow\PostgreSql\QueryBuilder\Schema\ColumnType::integer()))->from(table('users'))->toSql(),
            select(\Flow\PostgreSql\DSL\cast('id', \Flow\PostgreSql\QueryBuilder\Schema\ColumnType::integer()))->from('users')->toSql(),
        );
    }
    ...
    {
        self::assertSame(
            delete()->from('users')->using(table('orders'))->where(eq(col('users.id'), col('orders.user_id')))->toSql(),
    ...
    {
        self::assertSame(
            select(star())->from(table('users'))->where(eq(col('status'), col('other')))->toSql(),
    ...
    {
        self::assertSame(
            select(star())->from(table('users'))->where(\Flow\PostgreSql\DSL\is_null(col('email')))->toSql(),
            select(star())->from('users')->where(\Flow\PostgreSql\DSL\is_null('email'))->toSql(),
        );
    }
    ...
    {
        self::assertSame(
            select(star())->from(table('users'))->join(table('orders'), eq(col('users.id'), col('orders.user_id')))->toSql(),
    ...
    {
        self::assertSame(
            select(star())->from(table('users'))->leftJoin(table('orders'), eq(col('users.id'), col('orders.user_id')))->toSql(),
    ...
    {
        self::assertSame(
            select(star())->from(table('users'))->where(\Flow\PostgreSql\DSL\like(col('name'), literal('%test%')))->toSql(),
            select(star())->from('users')->where(\Flow\PostgreSql\DSL\like('name', literal('%test%')))->toSql(),
        );
    }
    ...
    {
        self::assertSame(
            delete()->from('users')->where(eq(col('id'), literal(1)))->returning(col('id'), col('name'))->toSql(),
            delete()->from('users')->where(eq('id', literal(1)))->returning(col('id'), col('name'))->toSql(),
        );
    }
    ...
    {
        self::assertSame(
            update()->update('users')->set('name', literal('test'))->from(table('logs'))->where(eq(col('users.id'), col('logs.user_id')))->toSql(),
            update()->update('users')->set('name', literal('test'))->from('logs')->where(eq('users.id', 'logs.user_id'))->toSql(),
        );
    }
}
