...
final class CreateIndexBuilderTest extends TestCase
{
    ...
    {
        $builder = CreateIndexBuilder::create('idx_users_email')->on('users')->columns(
            IndexColumn::column('email')->desc(),
        ...
        static::assertCount(2, $ast->getIndexParams());
    }

    public function test_columns_accepts_strings(): void
    {
        $builder = CreateIndexBuilder::create('idx_users_email')->on('users')->columns('email', 'name');
    ...
    {
        $builder = CreateIndexBuilder::create('idx_users_email')->concurrently()->on('users')->columns('email');
    ...
    {
        $builder = CreateIndexBuilder::create('idx_users_email')->ifNotExists()->on('users')->columns('email');
    ...
    {
        $original = CreateIndexBuilder::create('idx_users_email')->on('users')->columns('email');
        $modified = $original->tablespace('fast_storage');

        $originalAst = $original->toAst();
        $modifiedAst = $modified->toAst();

        static::assertEmpty($originalAst->getTableSpace());
        static::assertSame('fast_storage', $modifiedAst->getTableSpace());
    }
    ...
    public function test_include_columns(): void
    {
        ...
        static::assertInstanceOf(IndexStmt::class, $ast);
        static::assertCount(2, $ast->getIndexIncludingParams());
    }
    ...
    {
        $builder = CreateIndexBuilder::create('idx_users_email')->onOnly('users')->columns('email');
    ...
    {
        $builder = CreateIndexBuilder::create('idx_users_email')->on('users')->columns('email');
    ...
    public function test_tablespace(): void
    {
        ...
        static::assertInstanceOf(IndexStmt::class, $ast);
        static::assertSame('fast_storage', $ast->getTableSpace());
    }

    public function test_unique_sets_flag(): void
    {
        $builder = CreateIndexBuilder::create('idx_users_email')->unique()->on('users')->columns('email');

        $ast = $builder->toAst();

        static::assertInstanceOf(IndexStmt::class, $ast);
        static::assertTrue($ast->getUnique());
    }
    ...
    public function test_using_sets_access_method(): void
    {
        ...
        static::assertInstanceOf(IndexStmt::class, $ast);
        static::assertSame('hash', $ast->getAccessMethod());
    }

    public function test_with_schema(): void
    {
        $builder = CreateIndexBuilder::create('idx_users_email')->on('users', 'public')->columns('email');
