...
use function Flow\PostgreSql\DSL\agg_avg;
use function Flow\PostgreSql\DSL\agg_count;
use function Flow\PostgreSql\DSL\agg_max;
use function Flow\PostgreSql\DSL\agg_min;
use function Flow\PostgreSql\DSL\agg_sum;
use function Flow\PostgreSql\DSL\all_;
...
use function Flow\PostgreSql\DSL\distinct_from;
use function Flow\PostgreSql\DSL\eq;
use function Flow\PostgreSql\DSL\exists;
use function Flow\PostgreSql\DSL\func;
use function Flow\PostgreSql\DSL\ge;
use function Flow\PostgreSql\DSL\greatest;
...
use function Flow\PostgreSql\DSL\lateral;
use function Flow\PostgreSql\DSL\le;
use function Flow\PostgreSql\DSL\least;
...
use function Flow\PostgreSql\DSL\similar_to;
...
final class SelectBuilderTest extends TestCase
{
    ...
    {
        $builder1 = SelectBuilder::create()->select(Column::name('id'))->from(new Table('users'));
    ...
    {
        $builder1 = SelectBuilder::create()->select(Column::name('id'))->from(new Table('users'));
        $builder2 = $builder1->where(
            new Comparison(Column::name('active'), ComparisonOperator::EQ, Literal::bool(true)),
        );
    ...
    public function test_round_trip_simple(): void
    {
        ...
        static::assertEquals($ast->serializeToString(), $restoredAst->serializeToString());
    }
    ...
    public function test_round_trip_with_set_operation(): void
    {
        ...
        $query2 = SelectBuilder::create()->select(Column::name('id'))->from(new Table('table2'));
    ...
    {
        $query = SelectBuilder::create()->selectDistinct(Column::name('city'))->from(new Table('users'));

        $ast = $query->toAst();
        $distinctClause = $ast->getDistinctClause();

        static::assertNotNull($distinctClause);
    }
    ...
    public function test_select_distinct_on(): void
    {
        ...
        static::assertNotNull($distinctClause);
        static::assertCount(1, $distinctClause);
    }
    ...
    {
        $query = SelectBuilder::create()->select(Star::all())->from(new Table('users'), new Table('orders'));
    ...
    {
        $query = SelectBuilder::create()->select(Star::all())->from(new Table('users'))->limit(10);

        $ast = $query->toAst();
        $limitCount = $ast->getLimitCount();
        $limitOffset = $ast->getLimitOffset();

        static::assertNotNull($limitCount);
        static::assertNull($limitOffset);
    }

    public function test_select_only_offset(): void
    {
        $query = SelectBuilder::create()->select(Star::all())->from(new Table('users'))->offset(20);

        $ast = $query->toAst();
        $limitCount = $ast->getLimitCount();
        $limitOffset = $ast->getLimitOffset();

        static::assertNull($limitCount);
        static::assertNotNull($limitOffset);
    }

    public function test_select_star(): void
    {
        $query = SelectBuilder::create()->select(Star::all())->from(new Table('users'));

        $ast = $query->toAst();
        $targetList = $ast->getTargetList();

        static::assertNotNull($targetList);
        static::assertCount(1, $targetList);
    }
        ...
        static::assertSame(
            ...
            select()
                ...
                ->where(all_(
                    ...
                    select()->select(col('price'))->from(table('budget_products')),
                ))
                ->toSql(),
        );
    }
        ...
        static::assertSame(
            ...
            select()
                ...
                ->where(any_(
                    col('price'),
                    ComparisonOperator::GT,
                    select()->select(col('price'))->from(table('discounted_products')),
                ))
                ->toSql(),
        );
    }
    ...
    public function test_select_with_coalesce_to_sql(): void
    {
        static::assertSame(
            ...
            select()
                ->select(col('id'), coalesce(col('nickname'), col('name'), literal('Anonymous'))->as('display_name'))
                ->from(table('users'))
                ->toSql(),
        );
    }
        ...
        static::assertSame(
            ...
            select()
                ...
                ->where(and_(
                    ...
                    or_(ge(col('age'), literal(18)), eq(col('guardian_approved'), literal(true))),
                ))
                ->toSql(),
        );
    }
    ...
    {
        $query = SelectBuilder::create()->select(Star::all())->from(new Table('t1'))->crossJoin(new Table('t2'));
    ...
    {
        $query1 = SelectBuilder::create()->select(Column::name('id'))->from(new Table('table1'));
        ...
        static::assertNotNull($ast->getLarg());
        static::assertNotNull($ast->getRarg());
    }
    ...
    {
        $query = SelectBuilder::create()->select(Star::all())->from(new Table('users'))->forKeyShare();

        $ast = $query->toAst();
        $lockingClause = $ast->getLockingClause();

        static::assertNotNull($lockingClause);
        static::assertCount(1, $lockingClause);
    }

    public function test_select_with_for_no_key_update(): void
    {
        $query = SelectBuilder::create()->select(Star::all())->from(new Table('users'))->forNoKeyUpdate();
    ...
    {
        $query = SelectBuilder::create()->select(Star::all())->from(new Table('users'))->forShare();
    ...
    {
        $query = SelectBuilder::create()->select(Star::all())->from(new Table('users'))->forUpdate();
    ...
    {
        $query = SelectBuilder::create()->select(Star::all())->from(new Table('users'))->forUpdateSkipLocked();
    ...
    public function test_select_with_group_by_having(): void
    {
        ...
        static::assertNotNull($groupClause);
        static::assertCount(1, $groupClause);
        static::assertNotNull($havingClause);
    }
    ...
    public function test_select_with_is_distinct_from_to_sql(): void
    {
        static::assertSame(
            ...
            select()
                ->select(star())
                ->from(table('users'))
                ->where(distinct_from(col('status'), literal(null)))
                ->toSql(),
        );
    }
    ...
    public function test_select_with_is_not_distinct_from_to_sql(): void
    {
        static::assertSame(
            ...
            select()
                ->select(star())
                ->from(table('users'))
                ->where(distinct_from(col('status'), literal('active'), true))
                ->toSql(),
        );
    }
    ...
    public function test_select_with_not_condition_to_sql(): void
    {
        static::assertSame(
            ...
            select()
                ->select(star())
                ->from(table('users'))
                ->where(not_(eq(col('status'), literal('banned'))))
                ->toSql(),
        );
    }
    ...
    public function test_select_with_order_by_asc_desc_to_sql(): void
    {
        static::assertSame(
            ...
            select()
                ->select(star())
                ->from(table('users'))
                ->orderBy(asc(col('last_name')), desc(col('first_name')))
                ->toSql(),
        );
    }
    ...
    public function test_select_with_order_by_multiple_columns(): void
    {
        ...
        static::assertNotNull($sortClause);
        static::assertCount(2, $sortClause);
    }
    ...
    public function test_select_with_order_limit_offset(): void
    {
        ...
        static::assertNotNull($sortClause);
        static::assertCount(1, $sortClause);
        static::assertNotNull($limitCount);
        static::assertNotNull($limitOffset);
    }
    ...
    public function test_select_with_raw_expression_in_select_to_sql(): void
    {
        static::assertSame(
            ...
            select()
                ->select(col('id'), binary_expr(literal(1), '+', literal(1))->as('two'))
                ->from(table('users'))
                ->toSql(),
        );
    }
    ...
    public function test_select_with_recursive_cte_window_functions_and_case_expression_to_sql(): void
    {
        ...
        $orgTreeRecursive = select()
            ->select(
                ...
                binary_expr(binary_expr(col('org_tree.path'), '||', literal(' -> ')), '||', col('e.name')),
            )
        ...
        $deptStats = select()
            ->select(col('department_id'), agg_avg(col('salary')), agg_max(col('salary')), agg_count())
            ->from(table('employees'))
        ...
        $rankedEmployees = select()
            ->select(
                ...
                func('round', [
                    binary_expr(binary_expr(col('e.salary'), '/', col('ds.max_salary')), '*', literal(100)),
                    literal(2),
                ]),
            )
            ->from(table('employees')->as('e'))
        ...
        $query = with(cte('org_tree', $orgTreeQuery, [
            ...
            'level',
            'path',
        ]), cte('dept_stats', $deptStats, [
            'department_id',
            'avg_salary',
            'max_salary',
            'employee_count',
        ]), cte('ranked_employees', $rankedEmployees, [
            'id',
            'name',
            'department_id',
            'salary',
            ...
            ->where(and_(le(col('org_tree.level'), literal(3)), le(col('re.salary_rank'), literal(5))))
            ->orderBy(asc(col('org_tree.level')), asc(col('d.name')), desc(col('re.salary')));
        ...
        static::assertSame($expectedSql, $query->toSql());
    }
    ...
    {
        $query = SelectBuilder::create()->select(Star::all())->from(new Table('users', 'public'));
    ...
    {
        $query1 = SelectBuilder::create()->select(Column::name('name'))->from(new Table('users'));

        $query2 = SelectBuilder::create()->select(Column::name('name'))->from(new Table('admins'));
    ...
    {
        $query = SelectBuilder::create()->select(Column::name('id'), Column::name('name'))->from(new Table('users'));

        $ast = $query->toAst();
        static::assertInstanceOf(SelectStmt::class, $ast);
    ...
    {
        static::assertSame('SELECT * FROM users', select()->select(star())->from(table('users'))->toSql());
    }
    ...
    {
        static::assertSame('SELECT current_date AS today', select()->select(current_date()->as('today'))->toSql());
    }
