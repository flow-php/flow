...
use function Flow\PostgreSql\DSL\{schema_check, schema_column_integer, schema_column_text, schema_exclude, schema_foreign_key, schema_index, schema_primary_key, schema_table, schema_trigger, schema_unique};

use Flow\PostgreSql\QueryBuilder\Schema\{ColumnType, ReferentialAction};
use Flow\PostgreSql\Schema\{Column, IdentityGeneration, PartitionStrategy, TriggerEvent, TriggerTiming};
use Flow\PostgreSql\Schema\Constraint\{CheckConstraint as SchemaCheckConstraint, ForeignKey, UniqueConstraint as SchemaUniqueConstraint};
use Flow\PostgreSql\Schema\Diff\{ColumnDiff, TableDiff};
...
final class TableDiffTest extends TestCase
{
    public function test_adds_check_constraint() : void
    {
        ...
        $diff = new TableDiff(
            $table,
            $table,
            addedCheckConstraints: [schema_check('age > 0', 'chk_users_age')],
        );

        $sqls = $diff->generate();

        self::assertCount(1, $sqls);
        self::assertSame('ALTER TABLE public.users ADD CONSTRAINT chk_users_age CHECK (age > 0)', $sqls[0]->toSql());
    }
    ...
    public function test_adds_check_constraint_with_no_inherit() : void
    {
        ...
        $diff = new TableDiff(
            $source,
            $target,
            addedCheckConstraints: [new SchemaCheckConstraint('age > 0', 'chk_age', true)],
        );

        $sqls = $diff->generate();

        self::assertCount(1, $sqls);
        self::assertStringContainsString('NO INHERIT', $sqls[0]->toSql());
    }
    ...
    public function test_adds_column_with_generated_expression() : void
    {
        ...
        self::assertCount(1, $sqls);
        self::assertStringContainsString('ADD COLUMN', $sqls[0]->toSql());
    }
    ...
    public function test_adds_column_with_identity() : void
    {
        ...
        $diff = new TableDiff(
            $source,
            $target,
            [new Column('seq_id', ColumnType::integer(), false, isIdentity: true, identityGeneration: IdentityGeneration::ALWAYS)],
        );
    ...
    public function test_adds_column_with_not_null_and_default() : void
    {
        ...
        self::assertCount(1, $sqls);
        self::assertSame("ALTER TABLE public.users ADD COLUMN status pg_catalog.text NOT NULL DEFAULT 'active'", $sqls[0]->toSql());
    }
    ...
    public function test_adds_deferrable_foreign_key() : void
    {
        ...
        $diff = new TableDiff(
            $source,
            $target,
            addedForeignKeys: [new ForeignKey('fk_dept', ['dept_id'], 'public', 'departments', ['id'], ReferentialAction::NO_ACTION, ReferentialAction::CASCADE, true, true)],
        );

        $sqls = $diff->generate();

        self::assertCount(1, $sqls);
        self::assertStringContainsString('DEFERRABLE', $sqls[0]->toSql());
        self::assertStringContainsString('INITIALLY DEFERRED', $sqls[0]->toSql());
    }
    ...
    public function test_adds_exclude_constraint() : void
    {
        ...
        $diff = new TableDiff(
            $table,
            $table,
            addedExcludeConstraints: [schema_exclude('USING gist (tsrange(start_date, end_date) WITH &&)', 'excl_dates_overlap')],
        );
    ...
    public function test_adds_foreign_key() : void
    {
        ...
        $diff = new TableDiff(
            $table,
            $table,
            addedForeignKeys: [schema_foreign_key(['department_id'], 'departments', ['id'], 'fk_users_department')],
        );

        $sqls = $diff->generate();

        self::assertCount(1, $sqls);
        self::assertSame('ALTER TABLE public.users ADD CONSTRAINT fk_users_department FOREIGN KEY (department_id) REFERENCES public.departments (id)', $sqls[0]->toSql());
    }
    ...
    public function test_adds_index() : void
    {
        ...
        $diff = new TableDiff(
            $table,
            $table,
            addedIndexes: [schema_index('idx_users_name', ['name'])],
        );

        $sqls = $diff->generate();

        self::assertCount(1, $sqls);
        self::assertSame('CREATE INDEX idx_users_name ON public.users (name)', $sqls[0]->toSql());
    }
    ...
    public function test_adds_inherit() : void
    {
        ...
        self::assertCount(1, $sqls);
        self::assertSame('ALTER TABLE public.employees INHERIT persons', $sqls[0]->toSql());
    }
    ...
    public function test_adds_primary_key() : void
    {
        ...
        $diff = new TableDiff(
            $table,
            $table,
            addedPrimaryKey: schema_primary_key(['id'], 'pk_users'),
        );

        $sqls = $diff->generate();

        self::assertCount(1, $sqls);
        self::assertSame('ALTER TABLE public.users ADD CONSTRAINT pk_users PRIMARY KEY (id)', $sqls[0]->toSql());
    }
    ...
    public function test_adds_trigger() : void
    {
        ...
        $diff = new TableDiff(
            $table,
            $table,
            addedTriggers: [schema_trigger('trg_users_audit', 'users', TriggerTiming::AFTER, [TriggerEvent::INSERT], 'audit_function', true)],
        );

        $sqls = $diff->generate();

        self::assertCount(1, $sqls);
        self::assertSame('CREATE TRIGGER trg_users_audit AFTER INSERT ON public.users FOR EACH ROW EXECUTE FUNCTION audit_function()', $sqls[0]->toSql());
    }
    ...
    public function test_adds_trigger_with_before_timing() : void
    {
        ...
        $diff = new TableDiff(
            $table,
            $table,
            addedTriggers: [schema_trigger('trg_users_validate', 'users', TriggerTiming::BEFORE, [TriggerEvent::INSERT, TriggerEvent::UPDATE], 'validate_function', true)],
        );

        $sqls = $diff->generate();

        self::assertCount(1, $sqls);
        self::assertSame('CREATE TRIGGER trg_users_validate BEFORE INSERT OR UPDATE ON public.users FOR EACH ROW EXECUTE FUNCTION validate_function()', $sqls[0]->toSql());
    }
    ...
    public function test_adds_unique_constraint() : void
    {
        ...
        $diff = new TableDiff(
            $table,
            $table,
            addedUniqueConstraints: [schema_unique(['email'], 'uq_users_email')],
        );

        $sqls = $diff->generate();

        self::assertCount(1, $sqls);
        self::assertSame('ALTER TABLE public.users ADD CONSTRAINT uq_users_email UNIQUE (email)', $sqls[0]->toSql());
    }
    ...
    public function test_adds_unique_constraint_with_nulls_not_distinct() : void
    {
        ...
        $diff = new TableDiff(
            $source,
            $target,
            addedUniqueConstraints: [new SchemaUniqueConstraint(['email'], 'uq_email', true)],
        );

        $sqls = $diff->generate();

        self::assertCount(1, $sqls);
        self::assertStringContainsString('NULLS NOT DISTINCT', $sqls[0]->toSql());
    }
    ...
    public function test_changes_tablespace() : void
    {
        ...
        self::assertCount(1, $sqls);
        self::assertSame('ALTER TABLE public.users SET TABLESPACE fast_storage', $sqls[0]->toSql());
    }
    ...
    public function test_delegates_to_column_diff() : void
    {
        ...
        $diff = new TableDiff(
            $source,
            $target,
            modifiedColumns: [new ColumnDiff('public.users', schema_column_text('name'), schema_column_integer('name', false))],
        );

        $sqls = $diff->generate();

        self::assertCount(2, $sqls);
        self::assertSame('ALTER TABLE public.users ALTER COLUMN name TYPE int', $sqls[0]->toSql());
        self::assertSame('ALTER TABLE public.users ALTER COLUMN name SET NOT NULL', $sqls[1]->toSql());
    }
    ...
    public function test_dependency_order() : void
    {
        ...
        self::assertSame('ALTER TABLE public.users DROP CONSTRAINT fk_users_dept', $sqls[0]->toSql());
        self::assertSame('ALTER TABLE public.users ADD COLUMN email pg_catalog.text', $sqls[1]->toSql());
        self::assertSame('ALTER TABLE public.users ADD CONSTRAINT fk_users_email FOREIGN KEY (email) REFERENCES public.emails (address)', $sqls[2]->toSql());
    }
    ...
    public function test_does_not_require_view_rebuild_when_only_nullable_changed() : void
    {
        ...
        $target = schema_table('users', [schema_column_integer('id', false), schema_column_text('name', nullable: false)]);

        $diff = new TableDiff(
            $source,
            $target,
            modifiedColumns: [new ColumnDiff('public.users', schema_column_text('name'), schema_column_text('name', nullable: false))],
        );
    ...
    public function test_drops_check_constraint() : void
    {
        ...
        $diff = new TableDiff(
            $table,
            $table,
            removedCheckConstraints: [schema_check('age > 0', 'chk_users_age')],
        );

        $sqls = $diff->generate();

        self::assertCount(1, $sqls);
        self::assertSame('ALTER TABLE public.users DROP CONSTRAINT chk_users_age', $sqls[0]->toSql());
    }
    ...
    public function test_drops_column() : void
    {
        ...
        $diff = new TableDiff(
            $source,
            $target,
            removedColumns: [schema_column_text('name')],
        );

        $sqls = $diff->generate();

        self::assertCount(1, $sqls);
        self::assertSame('ALTER TABLE public.users DROP name', $sqls[0]->toSql());
    }
    ...
    public function test_drops_exclude_constraint() : void
    {
        ...
        $diff = new TableDiff(
            $table,
            $table,
            removedExcludeConstraints: [schema_exclude('USING gist (tsrange(start_date, end_date) WITH &&)', 'excl_dates_overlap')],
        );

        $sqls = $diff->generate();

        self::assertCount(1, $sqls);
        self::assertSame('ALTER TABLE public.users DROP CONSTRAINT excl_dates_overlap', $sqls[0]->toSql());
    }
    ...
    public function test_drops_foreign_key() : void
    {
        ...
        $diff = new TableDiff(
            $table,
            $table,
            removedForeignKeys: [schema_foreign_key(['department_id'], 'departments', ['id'], 'fk_users_department')],
        );

        $sqls = $diff->generate();

        self::assertCount(1, $sqls);
        self::assertSame('ALTER TABLE public.users DROP CONSTRAINT fk_users_department', $sqls[0]->toSql());
    }
    ...
    public function test_drops_index() : void
    {
        ...
        $diff = new TableDiff(
            $table,
            $table,
            removedIndexes: [schema_index('idx_users_name', ['name'])],
        );

        $sqls = $diff->generate();

        self::assertCount(1, $sqls);
        self::assertSame('DROP INDEX idx_users_name', $sqls[0]->toSql());
    }
    ...
    public function test_drops_primary_key() : void
    {
        ...
        $diff = new TableDiff(
            $table,
            $table,
            removedPrimaryKey: schema_primary_key(['id'], 'pk_users'),
        );

        $sqls = $diff->generate();

        self::assertCount(1, $sqls);
        self::assertSame('ALTER TABLE public.users DROP CONSTRAINT pk_users', $sqls[0]->toSql());
    }
    ...
    public function test_drops_trigger() : void
    {
        ...
        $diff = new TableDiff(
            $table,
            $table,
            removedTriggers: [schema_trigger('trg_users_audit', 'users', TriggerTiming::AFTER, [TriggerEvent::INSERT], 'audit_function', true)],
        );

        $sqls = $diff->generate();

        self::assertCount(1, $sqls);
        self::assertSame('DROP TRIGGER trg_users_audit ON public.users', $sqls[0]->toSql());
    }
    ...
    public function test_drops_unique_constraint() : void
    {
        ...
        $diff = new TableDiff(
            $table,
            $table,
            removedUniqueConstraints: [schema_unique(['email'], 'uq_users_email')],
        );

        $sqls = $diff->generate();

        self::assertCount(1, $sqls);
        self::assertSame('ALTER TABLE public.users DROP CONSTRAINT uq_users_email', $sqls[0]->toSql());
    }
    ...
    public function test_not_empty_when_column_modified() : void
    {
        ...
        $diff = new TableDiff(
            $source,
            $target,
            modifiedColumns: [new ColumnDiff('public.users', schema_column_text('name'), schema_column_text('name', false))],
        );
    ...
    public function test_not_empty_when_partition_changed() : void
    {
        ...
        $target = schema_table('events', [schema_column_integer('id', false)], partitionStrategy: PartitionStrategy::RANGE, partitionColumns: ['id']);
    ...
    public function test_partition_changed_throws_runtime_exception() : void
    {
        ...
        $this->expectExceptionMessage('Partition strategy change on table "public.events" cannot be applied via ALTER TABLE');
    ...
    public function test_removes_inherit() : void
    {
        ...
        self::assertCount(1, $sqls);
        self::assertSame('ALTER TABLE public.employees NO INHERIT persons', $sqls[0]->toSql());
    }
    ...
    public function test_renames_index() : void
    {
        ...
        $diff = new TableDiff(
            $table,
            $table,
            renamedIndexes: ['idx_users_name' => schema_index('idx_users_email', ['email'])],
        );

        $sqls = $diff->generate();

        self::assertCount(1, $sqls);
        self::assertSame('ALTER INDEX public.idx_users_name RENAME TO idx_users_email', $sqls[0]->toSql());
    }
    ...
    public function test_requires_view_rebuild_when_column_renamed() : void
    {
        ...
        $diff = new TableDiff(
            $source,
            $target,
            modifiedColumns: [new ColumnDiff('public.users', schema_column_text('name'), schema_column_text('full_name'))],
        );
    ...
    public function test_requires_view_rebuild_when_column_type_changes() : void
    {
        ...
        $diff = new TableDiff(
            $source,
            $target,
            modifiedColumns: [new ColumnDiff('public.users', schema_column_integer('amount'), schema_column_text('amount'))],
        );
    ...
    public function test_reversed_add_trigger() : void
    {
        ...
        $diff = new TableDiff(
            $table,
            $table,
            removedTriggers: [schema_trigger('trg_audit', 'users', TriggerTiming::AFTER, [TriggerEvent::INSERT], 'audit_fn')],
        );

        $sqls = $diff->generate();

        self::assertCount(1, $sqls);
        self::assertStringContainsString('DROP TRIGGER trg_audit', $sqls[0]->toSql());
    }
    ...
    public function test_reversed_drop_check_constraint() : void
    {
        ...
        $diff = new TableDiff(
            $table,
            $table,
            addedCheckConstraints: [schema_check('age > 0', 'chk_age')],
        );

        $sqls = $diff->generate();

        self::assertCount(1, $sqls);
        self::assertStringContainsString('CHECK', $sqls[0]->toSql());
    }
    ...
    public function test_reversed_drop_foreign_key() : void
    {
        ...
        $diff = new TableDiff(
            $table,
            $table,
            addedForeignKeys: [schema_foreign_key(['dept_id'], 'departments', ['id'], 'fk_dept')],
        );

        $sqls = $diff->generate();

        self::assertCount(1, $sqls);
        self::assertStringContainsString('FOREIGN KEY', $sqls[0]->toSql());
    }
    ...
    public function test_reversed_modified_column() : void
    {
        ...
        $diff = new TableDiff(
            $source,
            $target,
            modifiedColumns: [new ColumnDiff('public.users', schema_column_integer('name', false), schema_column_text('name'))],
        );

        $sqls = $diff->generate();

        self::assertCount(2, $sqls);
        self::assertStringContainsString('TYPE', $sqls[0]->toSql());
    }
    ...
    public function test_reversed_unlogged_change() : void
    {
        ...
        self::assertCount(1, $sqls);
        self::assertSame('ALTER TABLE public.users SET LOGGED', $sqls[0]->toSql());
    }
    ...
    public function test_sets_unlogged() : void
    {
        ...
        self::assertCount(1, $sqls);
        self::assertSame('ALTER TABLE public.users SET UNLOGGED', $sqls[0]->toSql());
    }
    ...
    public function test_throws_when_dropping_unnamed_check_constraint() : void
    {
        ...
        $diff = new TableDiff(
            $table,
            $table,
            removedCheckConstraints: [schema_check('age > 0')],
        );
    ...
    public function test_throws_when_dropping_unnamed_exclude_constraint() : void
    {
        ...
        $diff = new TableDiff(
            $table,
            $table,
            removedExcludeConstraints: [schema_exclude('USING gist (col WITH &&)')],
        );
    ...
    public function test_throws_when_dropping_unnamed_foreign_key() : void
    {
        ...
        $diff = new TableDiff(
            $table,
            $table,
            removedForeignKeys: [schema_foreign_key(['department_id'], 'departments', ['id'])],
        );
    ...
    public function test_throws_when_dropping_unnamed_primary_key() : void
    {
        ...
        $diff = new TableDiff(
            $table,
            $table,
            removedPrimaryKey: schema_primary_key(['id']),
        );
    ...
    public function test_throws_when_dropping_unnamed_unique_constraint() : void
    {
        ...
        $diff = new TableDiff(
            $table,
            $table,
            removedUniqueConstraints: [schema_unique(['email'])],
        );
