...
use function Flow\PostgreSql\DSL\ref_action_cascade;
use function Flow\PostgreSql\DSL\ref_action_restrict;
use function Flow\PostgreSql\DSL\ref_action_set_null;
...
final class TableBuilderTest extends TestCase
{
    ...
    {
        $builder = alter()->table('users')->addColumn(column('email', column_type_varchar(255))->notNull());

        static::assertSame('ALTER TABLE users ADD COLUMN email varchar(255) NOT NULL', $builder->toSql());
    }

    public function test_alter_table_add_constraint(): void
    {
        $builder = alter()->table('users')->addConstraint(unique_constraint('email')->name('users_email_unique'));

        static::assertSame('ALTER TABLE users ADD CONSTRAINT users_email_unique UNIQUE (email)', $builder->toSql());
    }

    public function test_alter_table_alter_column_set_default(): void
    {
        $builder = alter()->table('users')->alterColumnSetDefault('status', literal('active'));

        static::assertSame("ALTER TABLE users ALTER COLUMN status SET DEFAULT 'active'", $builder->toSql());
    }

    public function test_alter_table_alter_column_set_not_null(): void
    {
        $builder = alter()->table('users')->alterColumnSetNotNull('email');

        static::assertSame('ALTER TABLE users ALTER COLUMN email SET NOT NULL', $builder->toSql());
    }

    public function test_alter_table_alter_column_type(): void
    {
        $builder = alter()->table('users')->alterColumnType('name', column_type_text());

        static::assertSame('ALTER TABLE users ALTER COLUMN name TYPE pg_catalog.text', $builder->toSql());
    }

    public function test_alter_table_drop_column(): void
    {
        $builder = alter()->table('users')->dropColumn('temp_column');

        static::assertSame('ALTER TABLE users DROP temp_column', $builder->toSql());
    }

    public function test_alter_table_drop_column_cascade(): void
    {
        $builder = alter()->table('users')->dropColumn('temp_column', cascade: true);

        static::assertSame('ALTER TABLE users DROP temp_column CASCADE', $builder->toSql());
    }

    public function test_alter_table_drop_constraint(): void
    {
        $builder = alter()->table('users')->dropConstraint('users_email_unique');

        static::assertSame('ALTER TABLE users DROP CONSTRAINT users_email_unique', $builder->toSql());
    }
    ...
    public function test_alter_table_if_exists(): void
    {
        ...
        static::assertSame('ALTER TABLE IF EXISTS users ADD COLUMN email varchar(255)', $builder->toSql());
    }
    ...
    {
        $builder = alter()->table('users')->renameColumn('old_name', 'new_name');

        static::assertSame('ALTER TABLE users RENAME COLUMN old_name TO new_name', $builder->toSql());
    }

    public function test_alter_table_rename_column_if_exists(): void
    {
        $builder = alter()->table('users')->ifExists()->renameColumn('old_name', 'new_name');

        static::assertSame('ALTER TABLE IF EXISTS users RENAME COLUMN old_name TO new_name', $builder->toSql());
    }

    public function test_alter_table_rename_column_with_schema(): void
    {
        $builder = alter()->table('users', 'public')->renameColumn('old_name', 'new_name');

        static::assertSame('ALTER TABLE public.users RENAME COLUMN old_name TO new_name', $builder->toSql());
    }

    public function test_alter_table_rename_constraint(): void
    {
        $builder = alter()->table('users')->renameConstraint('old_constraint', 'new_constraint');

        static::assertSame('ALTER TABLE users RENAME CONSTRAINT old_constraint TO new_constraint', $builder->toSql());
    }

    public function test_alter_table_rename_to(): void
    {
        $builder = alter()->table('users')->renameTo('users_archive');

        static::assertSame('ALTER TABLE users RENAME TO users_archive', $builder->toSql());
    }

    public function test_alter_table_set_schema(): void
    {
        $builder = alter()->table('users')->setSchema('archive');

        static::assertSame('ALTER TABLE users SET SCHEMA archive', $builder->toSql());
    }

    public function test_alter_table_set_schema_if_exists(): void
    {
        $builder = alter()->table('users')->ifExists()->setSchema('archive');

        static::assertSame('ALTER TABLE IF EXISTS users SET SCHEMA archive', $builder->toSql());
    }

    public function test_alter_table_with_schema(): void
    {
        $builder = alter()->table('users', 'public')->addColumn(column('email', column_type_varchar(255)));

        static::assertSame('ALTER TABLE public.users ADD COLUMN email varchar(255)', $builder->toSql());
    }

    public function test_create_table_as_if_not_exists(): void
    {
        $selectBuilder = select()->select(col('id'), col('name'))->from(table('users'));

        $builder = create()->tableAs('users_backup', $selectBuilder)->ifNotExists();

        static::assertSame('CREATE TABLE IF NOT EXISTS users_backup AS SELECT id, name FROM users', $builder->toSql());
    }
    ...
    public function test_create_table_as_with_column_names(): void
    {
        ...
        $builder = create()->tableAs('users_backup', $selectBuilder)->columnNames('user_id', 'user_name');
    ...
    public function test_create_table_as_with_no_data(): void
    {
        ...
        $builder = create()->tableAs('users_backup', $selectBuilder)->withNoData();

        static::assertSame('CREATE TABLE users_backup AS SELECT id, name FROM users  WITH NO DATA', $builder->toSql());
    }
    ...
    public function test_create_table_simple(): void
    {
        ...
        static::assertSame('CREATE TABLE users (id serial PRIMARY KEY, name varchar(100) NOT NULL)', $builder->toSql());
    }
    ...
    public function test_create_table_with_foreign_key_set_null(): void
    {
        $builder = create()
            ...
            ->constraint(foreign_key(['user_id'], 'users', ['id'])->onDelete(ref_action_set_null()));
    ...
    {
        $builder = create()->table('users', 'public')->column(column('id', column_type_serial())->primaryKey());

        static::assertSame('CREATE TABLE public.users (id serial PRIMARY KEY)', $builder->toSql());
    }
    ...
    {
        $builder = drop()->table('users')->cascade();

        static::assertSame('DROP TABLE users CASCADE', $builder->toSql());
    }

    public function test_drop_table_if_exists(): void
    {
        $builder = drop()->table('users')->ifExists();

        static::assertSame('DROP TABLE IF EXISTS users', $builder->toSql());
    }

    public function test_drop_table_if_exists_cascade(): void
    {
        $builder = drop()->table('users')->ifExists()->cascade();

        static::assertSame('DROP TABLE IF EXISTS users CASCADE', $builder->toSql());
    }
    ...
    public function test_drop_table_multiple_tables(): void
    {
        ...
        static::assertSame('DROP TABLE users, orders, products', $builder->toSql());
    }

    public function test_drop_table_restrict(): void
    {
        $builder = drop()->table('users')->restrict();

        static::assertSame('DROP TABLE users', $builder->toSql());
    }
    ...
    public function test_drop_table_with_schema(): void
    {
        ...
        static::assertSame('DROP TABLE public.users', $builder->toSql());
    }
    ...
    public function test_simple_create_table_as(): void
    {
        ...
        static::assertSame('CREATE TABLE users_backup AS SELECT id, name FROM users', $builder->toSql());
    }

    public function test_truncate_cascade(): void
    {
        $builder = truncate_table('users')->cascade();

        static::assertSame('TRUNCATE users CASCADE', $builder->toSql());
    }
    ...
    public function test_truncate_multiple_tables(): void
    {
        ...
        static::assertSame('TRUNCATE users, orders, products', $builder->toSql());
    }

    public function test_truncate_restart_identity(): void
    {
        $builder = truncate_table('users')->restartIdentity();

        static::assertSame('TRUNCATE users RESTART IDENTITY', $builder->toSql());
    }

    public function test_truncate_restart_identity_cascade(): void
    {
        $builder = truncate_table('users')->restartIdentity()->cascade();

        static::assertSame('TRUNCATE users RESTART IDENTITY CASCADE', $builder->toSql());
    }

    public function test_truncate_restrict(): void
    {
        $builder = truncate_table('users')->restrict();

        static::assertSame('TRUNCATE users', $builder->toSql());
    }
    ...
    public function test_truncate_with_schema(): void
    {
        ...
        static::assertSame('TRUNCATE public.users', $builder->toSql());
    }
}
