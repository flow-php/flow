...
use function Flow\PostgreSql\DSL\{sql_parse, sql_query_depth};
...
final class QueryDepthTest extends TestCase
{
    ...
    {
        $depth = new QueryDepth(sql_parse('SELECT * FROM (SELECT * FROM (SELECT * FROM (SELECT * FROM t) AS a) AS b) AS c'));

        self::assertSame(4, $depth->depth());
    }
    ...
    {
        self::assertSame(1, sql_query_depth('SELECT * FROM t'));
        self::assertSame(2, sql_query_depth('SELECT * FROM (SELECT * FROM t) AS sub'));
        self::assertSame(4, sql_query_depth('SELECT * FROM (SELECT * FROM (SELECT * FROM (SELECT * FROM t) AS a) AS b) AS c'));
    }
}
