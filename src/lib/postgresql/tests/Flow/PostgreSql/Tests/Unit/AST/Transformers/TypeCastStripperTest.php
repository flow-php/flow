...
final class TypeCastStripperTest extends TestCase
{
    ...
    {
        static::assertSame('name = 1 AND status = 2', $this->normalize('name::int = 1 AND status::int = 2'));
    }
    ...
    {
        static::assertSame("COALESCE(name, 'n/a')", $this->normalize("COALESCE((name)::text, 'n/a'::text)"));
    }

    public function test_strip_leaves_expression_without_casts_untouched(): void
    {
        static::assertSame("(first_name || ' ') || last_name", $this->normalize("first_name || ' ' || last_name"));
    }
    ...
    {
        static::assertSame('name IS NULL', $this->normalize('(name)::text IS NULL'));
    }

    public function test_strip_row_expression_elements(): void
    {
        static::assertSame("ROW(1, 'a')", $this->normalize("ROW(1::int, 'a'::text)"));
    }

    public function test_strip_stacked_casts(): void
    {
        static::assertSame('name', $this->normalize('((name)::text)::varchar'));
    }

    public function test_strip_top_level_cast(): void
    {
        static::assertSame('name', $this->normalize('(name)::text'));
    }
