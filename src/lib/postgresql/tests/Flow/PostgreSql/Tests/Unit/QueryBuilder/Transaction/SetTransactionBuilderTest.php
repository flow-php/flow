...
final class SetTransactionBuilderTest extends TestCase
{
    ...
    public function test_set_session_transaction_deparsed(): void
    {
        ...
        $query = SetTransactionBuilder::session()->isolationLevel(IsolationLevel::SERIALIZABLE);

        $deparsed = $this->deparse($query->toAst());
        static::assertSame('SET SESSION CHARACTERISTICS AS TRANSACTION ISOLATION LEVEL SERIALIZABLE', $deparsed);
    }

    public function test_set_transaction_ast_has_correct_kind(): void
    {
        $query = SetTransactionBuilder::create()->isolationLevel(IsolationLevel::SERIALIZABLE);

        $ast = $query->toAst();

        static::assertInstanceOf(VariableSetStmt::class, $ast);
        static::assertSame(VariableSetKind::VAR_SET_MULTI, $ast->getKind());
        static::assertSame('TRANSACTION', $ast->getName());
    }
    ...
    public function test_set_transaction_deparsed_with_isolation_level(): void
    {
        ...
        $query = SetTransactionBuilder::create()->isolationLevel(IsolationLevel::READ_COMMITTED);

        $deparsed = $this->deparse($query->toAst());
        static::assertSame('SET TRANSACTION ISOLATION LEVEL READ COMMITTED', $deparsed);
    }
    ...
    public function test_set_transaction_deparsed_with_multiple_options(): void
    {
        ...
        $deparsed = $this->deparse($query->toAst());
        static::assertSame('SET TRANSACTION ISOLATION LEVEL SERIALIZABLE, READ ONLY, DEFERRABLE', $deparsed);
    }
    ...
    public function test_set_transaction_deparsed_with_read_only(): void
    {
        ...
        $query = SetTransactionBuilder::create()->readOnly();

        $deparsed = $this->deparse($query->toAst());
        static::assertSame('SET TRANSACTION READ ONLY', $deparsed);
    }
    ...
    public function test_set_transaction_deparsed_with_read_write(): void
    {
        ...
        $query = SetTransactionBuilder::create()->readWrite();

        $deparsed = $this->deparse($query->toAst());
        static::assertSame('SET TRANSACTION READ WRITE', $deparsed);
    }
    ...
    public function test_set_transaction_snapshot_deparsed(): void
    {
        ...
        $query = SetTransactionBuilder::create()->snapshot('00000003-0000001A-1');

        $deparsed = $this->deparse($query->toAst());
        static::assertSame("SET TRANSACTION SNAPSHOT '00000003-0000001A-1'", $deparsed);
    }
