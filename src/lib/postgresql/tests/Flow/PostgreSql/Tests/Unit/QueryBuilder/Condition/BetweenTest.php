...
final class BetweenTest extends TestCase
{
    ...
    {
        $between = new Between(Column::name('age'), Literal::int(18), Literal::int(65));
        ...
        static::assertSame(A_Expr_Kind::AEXPR_BETWEEN, $aExpr->getKind());
    ...
    {
        $between = new Between(Column::name('score'), Literal::int(0), Literal::int(50), false, true);
        ...
        $aExpr = $ast->getAExpr();
        static::assertNotNull($aExpr);
        static::assertSame(A_Expr_Kind::AEXPR_NOT_BETWEEN, $aExpr->getKind());
    }

    public function test_between_symmetric_negated_to_ast(): void
    {
        $between = new Between(Column::name('value'), Literal::int(1), Literal::int(100), true, true);
        ...
        $aExpr = $ast->getAExpr();
        static::assertNotNull($aExpr);
        static::assertSame(A_Expr_Kind::AEXPR_NOT_BETWEEN_SYM, $aExpr->getKind());
    }

    public function test_between_symmetric_to_ast(): void
    {
        $between = new Between(Column::name('price'), Literal::float(10.0), Literal::float(100.0), true, false);
        ...
        $aExpr = $ast->getAExpr();
        static::assertNotNull($aExpr);
        static::assertSame(A_Expr_Kind::AEXPR_BETWEEN_SYM, $aExpr->getKind());
    }

    public function test_from_ast_reconstructs_between(): void
    {
        $original = new Between(Column::name('date'), Literal::string('2024-01-01'), Literal::string('2024-12-31'));

        $ast = $original->toAst();
        $reconstructed = Between::fromAst($ast);

        static::assertInstanceOf(Between::class, $reconstructed);
        static::assertFalse($reconstructed->symmetric);
    ...
    {
        $original = new Between(Column::name('id'), Literal::int(1), Literal::int(10), false, true);
    ...
    {
        $original = new Between(Column::name('value'), Literal::int(10), Literal::int(20), true, false);

        $ast = $original->toAst();
        $reconstructed = Between::fromAst($ast);

        static::assertInstanceOf(Between::class, $reconstructed);
        static::assertTrue($reconstructed->symmetric);
        static::assertFalse($reconstructed->negated);
    }
