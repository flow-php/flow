...
final class NotifyBuilderTest extends TestCase
{
    ...
    public function test_ast_contains_channel_name(): void
    {
        ...
        static::assertInstanceOf(NotifyStmt::class, $ast);
        static::assertSame('my_channel', $ast->getConditionname());
        static::assertSame('', $ast->getPayload());
    }
    ...
    public function test_ast_contains_payload_when_set(): void
    {
        ...
        static::assertSame('my_channel', $ast->getConditionname());
        static::assertSame('hello', $ast->getPayload());
    }

    public function test_deparses_channel_with_spaces_as_quoted_identifier(): void
    {
        static::assertSame('NOTIFY "weird name"', notify('weird name')->toSql());
    }

    public function test_deparses_simple_channel_without_payload(): void
    {
        static::assertSame('NOTIFY my_channel', notify('my_channel')->toSql());
    }

    public function test_deparses_with_payload(): void
    {
        static::assertSame("NOTIFY my_channel, 'hello'", notify('my_channel')->withPayload('hello')->toSql());
    }

    public function test_empty_payload_is_treated_as_no_payload(): void
    {
        static::assertSame('NOTIFY my_channel', notify('my_channel')->withPayload('')->toSql());
    }
    ...
    public function test_immutability_of_with_payload(): void
    {
        ...
        static::assertSame('NOTIFY my_channel', $original->toSql());
        static::assertSame("NOTIFY my_channel, 'hello'", $modified->toSql());
    }

    public function test_payload_with_single_quote_is_escaped(): void
    {
        static::assertSame("NOTIFY my_channel, 'it''s fine'", notify('my_channel')->withPayload("it's fine")->toSql());
    }
}
