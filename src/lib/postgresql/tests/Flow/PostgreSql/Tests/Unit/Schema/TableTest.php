...
use function Flow\PostgreSql\DSL\{column_type_integer, column_type_jsonb, column_type_text, column_type_timestamptz, column_type_varchar, schema_check, schema_column, schema_foreign_key, schema_index, schema_primary_key, schema_table, schema_trigger, schema_unique};
use Flow\PostgreSql\QueryBuilder\Schema\ReferentialAction;
use Flow\PostgreSql\Schema\Exception\ColumnNotFoundException;
use Flow\PostgreSql\Schema\{IdentityGeneration, IndexMethod, PartitionStrategy, TriggerEvent, TriggerTiming};
...
final class TableTest extends TestCase
{
    public function test_column_lookup() : void
    {
        ...
        self::assertSame('name', $table->column('name')->name);
    }
    ...
    public function test_qualified_name() : void
    {
        ...
        self::assertSame('public.users', $table->qualifiedName());
    }
    ...
    public function test_table_with_foreign_keys() : void
    {
        ...
        self::assertSame('users', $table->foreignKeys[0]->referenceTable);
    }
    ...
    public function test_table_with_indexes() : void
    {
        ...
        self::assertSame('idx_users_email', $table->indexes[0]->name);
    ...
    public function test_table_with_triggers() : void
    {
        $table = schema_table('users', [
            ...
            schema_trigger('trg_update_timestamp', 'users', TriggerTiming::BEFORE, [TriggerEvent::UPDATE], 'update_timestamp', forEachRow: true),
        ]);

        self::assertCount(1, $table->triggers);
        self::assertSame('trg_update_timestamp', $table->triggers[0]->name);
    }
    ...
    public function test_to_sql_generates_create_table() : void
    {
        ...
        self::assertSame(
            'CREATE TABLE public.users (id int NOT NULL, name varchar(255))',
            $sqls[0]->toSql(),
        );
    }
    ...
    public function test_to_sql_generates_create_table_with_indexes() : void
    {
        ...
        self::assertSame('CREATE TABLE public.users (id int NOT NULL, email varchar(255))', $sqls[0]->toSql());
        self::assertSame('CREATE UNIQUE INDEX idx_users_email ON public.users (email)', $sqls[1]->toSql());
    }
    ...
    public function test_to_sql_generates_create_table_with_primary_key() : void
    {
        $sqls = schema_table('users', [
            schema_column('id', column_type_integer(), nullable: false),
        ], primaryKey: schema_primary_key(['id']))->toSql();

        self::assertCount(1, $sqls);
        self::assertSame(
            'CREATE TABLE public.users (id int NOT NULL, PRIMARY KEY (id))',
            $sqls[0]->toSql(),
        );
    }
    ...
    {
        $sqls = schema_table('employees', [
            schema_column('id', column_type_integer(), nullable: false),
            schema_column('name', column_type_varchar(255)),
        ], inherits: ['persons'])->toSql();
    ...
    {
        $sqls = schema_table('events', [
            schema_column('id', column_type_integer(), nullable: false),
        ], partitionStrategy: PartitionStrategy::HASH, partitionColumns: ['id'])->toSql();
    ...
    public function test_to_sql_generates_partition_by_list() : void
    {
        ...
        ], partitionStrategy: PartitionStrategy::LIST, partitionColumns: ['region'])->toSql();
    ...
    public function test_to_sql_generates_partition_by_range() : void
    {
        ...
        ], partitionStrategy: PartitionStrategy::RANGE, partitionColumns: ['name'])->toSql();
    ...
    public function test_to_sql_generates_partition_with_tablespace() : void
    {
        $sqls = schema_table('events', [
            schema_column('id', column_type_integer(), nullable: false),
            schema_column('name', column_type_varchar(255), nullable: false),
        ], partitionStrategy: PartitionStrategy::RANGE, partitionColumns: ['name'], tablespace: 'fast_storage')->toSql();
    ...
    public function test_to_sql_with_column_default_numeric() : void
    {
        ...
        self::assertCount(1, $sqls);
        self::assertSame(
            'CREATE TABLE public.products (id int NOT NULL, quantity int DEFAULT 0)',
            $sqls[0]->toSql(),
        );
    }
    ...
    public function test_to_sql_with_column_generated_expression() : void
    {
        $sqls = schema_table('users', [
            ...
            schema_column('full_name', column_type_text(), isGenerated: true, generationExpression: "first_name || ' ' || last_name"),
        ])->toSql();
    ...
    {
        $sqls = schema_table('users', [
            schema_column('id', column_type_integer(), nullable: false, isIdentity: true, identityGeneration: IdentityGeneration::ALWAYS),
            schema_column('name', column_type_varchar(255)),
        ])->toSql();
    ...
    public function test_to_sql_with_custom_schema() : void
    {
        $sqls = schema_table('users', [
            schema_column('id', column_type_integer(), nullable: false),
        ], schema: 'myschema')->toSql();

        self::assertCount(1, $sqls);
        self::assertSame(
            'CREATE TABLE myschema.users (id int NOT NULL)',
            $sqls[0]->toSql(),
        );
    }
    ...
    public function test_to_sql_with_gist_index() : void
    {
        ...
        self::assertCount(2, $sqls);
        self::assertSame(
            'CREATE INDEX idx_events_range ON public.events USING gist (range_col)',
            $sqls[1]->toSql(),
        );
    }
    ...
    public function test_to_sql_with_multiple_indexes() : void
    {
        ...
        self::assertCount(3, $sqls);
        self::assertSame('CREATE UNIQUE INDEX idx_users_email ON public.users (email)', $sqls[1]->toSql());
        self::assertSame('CREATE INDEX idx_users_name ON public.users (name)', $sqls[2]->toSql());
    }
    ...
    public function test_to_sql_with_multiple_triggers() : void
    {
        $sqls = schema_table('users', [
            ...
            schema_trigger('trg_before', 'users', TriggerTiming::BEFORE, [TriggerEvent::INSERT], 'before_func', forEachRow: true),
            schema_trigger('trg_after', 'users', TriggerTiming::AFTER, [TriggerEvent::UPDATE], 'after_func'),
        ])->toSql();

        self::assertCount(3, $sqls);
        self::assertSame('CREATE TRIGGER trg_before BEFORE INSERT ON public.users FOR EACH ROW EXECUTE FUNCTION before_func()', $sqls[1]->toSql());
        self::assertSame('CREATE TRIGGER trg_after AFTER UPDATE ON public.users EXECUTE FUNCTION after_func()', $sqls[2]->toSql());
    }
    ...
    public function test_to_sql_with_named_primary_key() : void
    {
        $sqls = schema_table('users', [
            schema_column('id', column_type_integer(), nullable: false),
        ], primaryKey: schema_primary_key(['id'], name: 'pk_users'))->toSql();
    ...
    public function test_to_sql_with_trigger_before_delete() : void
    {
        $sqls = schema_table('users', [
            ...
            schema_trigger('trg_prevent_delete', 'users', TriggerTiming::BEFORE, [TriggerEvent::DELETE], 'prevent_delete_func', forEachRow: true),
        ])->toSql();
    ...
    {
        $sqls = schema_table('users_view', [
            schema_column('id', column_type_integer(), nullable: false),
        ], triggers: [
            schema_trigger('trg_view_insert', 'users_view', TriggerTiming::INSTEAD_OF, [TriggerEvent::INSERT], 'insert_to_users', forEachRow: true),
        ])->toSql();
    ...
    public function test_to_sql_with_trigger_multiple_events() : void
    {
        $sqls = schema_table('users', [
            ...
            schema_trigger('trg_audit', 'users', TriggerTiming::AFTER, [TriggerEvent::INSERT, TriggerEvent::UPDATE], 'audit_function', forEachRow: true),
        ])->toSql();
