...
use function Flow\PostgreSql\DSL\{alter, create, drop, grant, grant_role, reassign_owned, reset_role, revoke, revoke_role, set_role};
use Flow\PostgreSql\Protobuf\AST\{AlterRoleStmt, CreateRoleStmt, DropRoleStmt, ObjectType, RenameStmt, RoleSpecType, RoleStmtType};
use Flow\PostgreSql\QueryBuilder\Schema\Grant\TablePrivilege;
use Flow\PostgreSql\QueryBuilder\Schema\Role\{AlterRoleBuilder, CreateRoleBuilder, DropRoleBuilder};
...
final class RoleBuilderTest extends TestCase
{
    ...
    {
        self::assertSame('ALTER ROLE old_name RENAME TO new_name', alter()->role('old_name')->renameTo('new_name')->toSql());
    }

    public function test_alter_role_set_options_to_sql() : void
    {
        self::assertSame('ALTER ROLE admin WITH SUPERUSER', alter()->role('admin')->superuser()->toSql());
    }
    ...
    {
        self::assertSame('ALTER ROLE admin WITH NOSUPERUSER CREATEDB', alter()->role('admin')->noSuperuser()->createDb()->toSql());
    }
    ...
    {
        self::assertSame('CREATE ROLE admin', create()->role('admin')->toSql());
    }
    ...
    {
        self::assertSame('CREATE ROLE app_user WITH LOGIN', create()->role('app_user')->login()->toSql());
    }
    ...
    {
        self::assertSame('CREATE ROLE admin WITH SUPERUSER LOGIN', create()->role('admin')->superuser()->login()->toSql());
    }
    ...
    {
        self::assertSame("CREATE ROLE app_user WITH LOGIN PASSWORD 'secret'", create()->role('app_user')->login()->withPassword('secret')->toSql());
    }

    public function test_drop_owned_by_cascade_to_sql() : void
    {
        self::assertSame('DROP OWNED BY role1 CASCADE', drop()->owned('role1')->cascade()->toSql());
    }

    public function test_drop_owned_by_to_sql() : void
    {
        self::assertSame('DROP OWNED BY role1', drop()->owned('role1')->toSql());
    }
    ...
    {
        self::assertSame('DROP ROLE IF EXISTS admin', drop()->role('admin')->ifExists()->toSql());
    }
    ...
    {
        self::assertSame('DROP ROLE role1, role2', drop()->role('role1', 'role2')->toSql());
    }
    ...
    {
        self::assertSame('DROP ROLE admin', drop()->role('admin')->toSql());
    }

    public function test_grant_all_on_table_to_sql() : void
    {
        self::assertSame('GRANT ALL ON users TO admin', grant(TablePrivilege::ALL)->onTable('users')->to('admin')->toSql());
    }

    public function test_grant_on_all_tables_in_schema_to_sql() : void
    {
        self::assertSame('GRANT select ON ALL TABLES IN SCHEMA public TO admin', grant(TablePrivilege::SELECT)->onAllTablesInSchema('public')->to('admin')->toSql());
    }

    public function test_grant_role_to_role_to_sql() : void
    {
        self::assertSame('GRANT admin TO user1', grant_role('admin')->to('user1')->toSql());
    }

    public function test_grant_role_with_admin_option_to_sql() : void
    {
        self::assertSame('GRANT admin TO user1 WITH ADMIN OPTION', grant_role('admin')->to('user1')->withAdminOption()->toSql());
    }

    public function test_grant_select_on_table_to_sql() : void
    {
        self::assertSame('GRANT select ON users TO app_user', grant(TablePrivilege::SELECT)->onTable('users')->to('app_user')->toSql());
    }

    public function test_grant_with_grant_option_to_sql() : void
    {
        self::assertSame('GRANT select ON users TO app_user WITH GRANT OPTION', grant(TablePrivilege::SELECT)->onTable('users')->to('app_user')->withGrantOption()->toSql());
    }

    public function test_reassign_owned_by_to_sql() : void
    {
        self::assertSame('REASSIGN OWNED BY old_role TO new_role', reassign_owned('old_role')->to('new_role')->toSql());
    }

    public function test_reset_role_to_sql() : void
    {
        self::assertSame('RESET role', reset_role()->toSql());
    }

    public function test_revoke_role_cascade_to_sql() : void
    {
        self::assertSame('REVOKE admin FROM user1 CASCADE', revoke_role('admin')->from('user1')->cascade()->toSql());
    }

    public function test_revoke_role_from_role_to_sql() : void
    {
        self::assertSame('REVOKE admin FROM user1', revoke_role('admin')->from('user1')->toSql());
    }

    public function test_revoke_select_on_table_to_sql() : void
    {
        self::assertSame('REVOKE select ON users FROM app_user', revoke(TablePrivilege::SELECT)->onTable('users')->from('app_user')->toSql());
    }

    public function test_revoke_with_cascade_to_sql() : void
    {
        self::assertSame('REVOKE select ON users FROM app_user CASCADE', revoke(TablePrivilege::SELECT)->onTable('users')->from('app_user')->cascade()->toSql());
    }

    public function test_set_role_to_sql() : void
    {
        self::assertSame('SET role TO admin', set_role('admin')->toSql());
    }
}
