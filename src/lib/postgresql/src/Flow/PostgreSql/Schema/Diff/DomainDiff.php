...
final readonly class DomainDiff implements Diff
{
    ...
    public function generate() : array
    {
        ...
        foreach ($this->removedCheckConstraints as $cc) {
            if ($cc->name === null) {
                throw new \RuntimeException(\sprintf('Cannot drop unnamed check constraint on domain "%s". Constraint names are required for reversible migrations.', $this->target->name));
            }
