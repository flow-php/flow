...
class MigrationException extends \RuntimeException
{
    ...
    {
        return new self(\sprintf('Configuration file "%s" not found. Create a %s file or specify the path explicitly.', $fileName, $fileName));
    }
    ...
    {
        return new self(\sprintf('Invalid migration version format: "%s". Expected non-empty alphanumeric string (max 255 chars).', $version));
    }
