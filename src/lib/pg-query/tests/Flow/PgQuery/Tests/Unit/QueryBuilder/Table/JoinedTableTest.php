...
use Flow\PgQuery\Protobuf\AST\JoinType as ProtobufJoinType;
use Flow\PgQuery\QueryBuilder\Condition\{Comparison, ComparisonOperator};
use Flow\PgQuery\QueryBuilder\Expression\Column;
use Flow\PgQuery\QueryBuilder\Table\{AliasedTable, JoinType, JoinedTable, Table};
...
final class JoinedTableTest extends TestCase
{
    ...
    public function test_converts_inner_join_with_on_condition_to_ast() : void
    {
        ...
        self::assertInstanceOf(\Flow\PgQuery\Protobuf\AST\JoinExpr::class, $joinExpr);
        self::assertSame(ProtobufJoinType::JOIN_INNER, $joinExpr->getJointype());

        $larg = $joinExpr->getLarg();
        self::assertInstanceOf(\Flow\PgQuery\Protobuf\AST\Node::class, $larg);
        self::assertTrue($larg->hasRangeVar());
        $largRangeVar = $larg->getRangeVar();
        self::assertInstanceOf(\Flow\PgQuery\Protobuf\AST\RangeVar::class, $largRangeVar);
        self::assertSame('users', $largRangeVar->getRelname());

        $rarg = $joinExpr->getRarg();
        self::assertInstanceOf(\Flow\PgQuery\Protobuf\AST\Node::class, $rarg);
        self::assertTrue($rarg->hasRangeVar());
        $rargRangeVar = $rarg->getRangeVar();
        self::assertInstanceOf(\Flow\PgQuery\Protobuf\AST\RangeVar::class, $rargRangeVar);
        self::assertSame('orders', $rargRangeVar->getRelname());

        $quals = $joinExpr->getQuals();
        self::assertInstanceOf(\Flow\PgQuery\Protobuf\AST\Node::class, $quals);
    ...
    public function test_converts_using_clause_to_ast() : void
    {
        ...
        self::assertInstanceOf(\Flow\PgQuery\Protobuf\AST\PBString::class, $col);
