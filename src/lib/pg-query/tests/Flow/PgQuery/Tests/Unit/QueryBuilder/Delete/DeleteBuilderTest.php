...
namespace Flow\PgQuery\Tests\Unit\QueryBuilder\Delete;

use Flow\PgQuery\{ParsedQuery, Parser};
use Flow\PgQuery\Protobuf\AST\{DeleteStmt, Node, RawStmt, SelectStmt};
...
final class DeleteBuilderTest extends TestCase
{
    ...
    {
        $this->expectException(\Flow\PgQuery\QueryBuilder\Exception\InvalidAstException::class);
        $this->expectExceptionMessage('Missing required field "relname" in RangeVar node');

        $deleteStmt = new DeleteStmt();
        $deleteStmt->setRelation(new \Flow\PgQuery\Protobuf\AST\RangeVar([
            'relname' => '',
        ]));
