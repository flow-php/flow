...
use function Flow\Types\DSL\{type_enum, type_scalar, type_string, type_union};
...
final readonly class Options
{
    ...
    public function __construct(array $options)
    {
        ...
        foreach ($options as $option => $value) {
            $normalizedOptions[\mb_strtolower(type_string()->cast($option))] = type_union(type_scalar(), type_enum(\UnitEnum::class))->assert($value);
        }

        $this->options = $normalizedOptions;
    }

    public function get(string|Option $option, string|int|bool|float|\UnitEnum|null $default = null) : string|int|bool|float|\UnitEnum|null
    {
        if ($this->has($option)) {
