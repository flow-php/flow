...
final class MySQLDbalQueryExtractorTest extends IntegrationTestCase
{
    public function test_extracting_multiple_rows_at_once(): void
    {
        ...
        data_frame()
            ...
            ->load(DbalLoader::fromConnection($this->mysqlDatabaseContext->connection(), $table))
            ->run();

        $rows = df()
            ->extract(from_dbal_query($this->mysqlDatabaseContext->connection(), "SELECT * FROM {$table} ORDER BY id"))
            ->fetch();
