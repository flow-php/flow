...
final class SqliteKeySetExtractorTest extends IntegrationTestCase
{
    public function test_extracting_empty_table(): void
    {
        ...
        $rows = data_frame()
            ->extract(from_dbal_key_set_qb(
                ...
                $this->sqliteDatabaseContext->connection()->createQueryBuilder()->from($table)->select('*'),
    ...
    public function test_extraction_when_key_is_ambiguous_column(): void
    {
        ...
        for ($i = 1; $i <= 25; $i++) {
            ...
            $this->sqliteDatabaseContext->insert('flow_key_set_extractor_test_02', [
                'id' => $i,
                'id_01' => $i,
                'name' => 'name_' . $i,
            ]);
