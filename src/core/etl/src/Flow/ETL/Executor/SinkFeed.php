...
/**
 * The planner's step that feeds one sink pipeline: a non-bare sink root, or a node several sinks share.
 */
final class SinkFeed implements Closure, Discardable, Loader
{
    ...
    /**
     * @param Loader ...$consumers the sink pipeline's last steps, which a completed run's discard() is forwarded to
     *
     * @throws InvalidArgumentException when no consumer is given
     */
    public function __construct(
        private readonly FeedExtractor $feed,
        private readonly SinkRun $run,
        private readonly SinkOffers $offers,
        Loader ...$consumers,
    ) {
    ...
    public function load(Rows $rows, FlowContext $context): void
    {
        ...
        // offered INSIDE the sink pipeline -> wrapped, so the outer Segment does not offer it twice. An ENDING
        // failure (a limit inside the sink pipeline completed it mid-load) was offered to nobody -> raw, so the outer
        // Segment offers it exactly once, against this step.
        try {
            $this->run->advance();
        } catch (Throwable $failure) {
            throw $this->offers->offered($failure) ? new SinkFailure($failure) : $failure;
        }
    }
    ...
    /**
     * After a rollback: the next batch runs a fresh fiber over the same sink pipeline.
     */
