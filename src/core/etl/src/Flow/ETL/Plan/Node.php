...
{
    /**
     * Inputs, in lowering order. children()[0] is the node's ROW INPUT - the chain a rule walks down and
     * the chain PipelineSplit turns into segments. Every further child is a SIDE INPUT and is always a
     * Frame: another plan this node reads without taking its rows as input. A leaf returns [].
     *
     * Three kinds break that shape. A Frame is a LEAF that owns a whole plan - read it through Frame::plan(),
     * a rule over the outer plan never descends into it. A Transaction is a ROOT-ONLY node whose children
     * are sibling sink roots it commits together - never a row input, never on a spine. A SinkMultiple is the ONE
     ...
     * an untouched subtree keeps its identity and its analysis (Analysis memoises by identity).
     *
     ...
     *
     * A node whose lowering produces only Transformers is streaming by construction: Segment::execute()
