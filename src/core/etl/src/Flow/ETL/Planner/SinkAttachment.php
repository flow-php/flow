 ...
 * the other methods are the walk it runs, @internal. SideFeed builds the pipelines the walk hands rows to.
 *
...
final class SinkAttachment
{
    ...
    private readonly SideFeed $feed;

    public function __construct(
        private readonly Analysis $analysis,
        private readonly FlowContext $context,
    ) {
        $this->feed = new SideFeed($analysis, $context);
    }
     ...
     *
     * @throws InvalidLogicException when a sink shares no node with the spine, a Write does not lower to a Loader,
     *                               or the sinks of one transaction attach to different nodes
     *
     * @return SplObjectStorage<Node, list<Loader>>
     */
    public function attach(array $sinks, SplObjectStorage $onSpine): SplObjectStorage
    {
        ...
        foreach ($sinks as $sink) {
            ...
            foreach ($sink instanceof Transaction ? $sink->children() : [$sink] as $write) {
                ...
                $at = $bottom;
                $loader = $this->analysis->steps($write, $this->context)[0] ?? null;

                if (!$loader instanceof Loader) {
                    throw InvalidLogicException::because(
                        'A Write must lower to a Loader, %s given',
                        get_debug_type($loader),
                    );
                }

                $attached[$bottom] = [
                    ...($attached->contains($bottom) ? $attached[$bottom] : []),
                    [
                        'write' => $write,
                        'loader' => $loader,
    ...
    public function chain(Write $write, SplObjectStorage $onSpine): array
    {
        ...
        for ($node = $write->children()[0]; !$onSpine->contains($node); $node = $node->children()[0]) {
            $chain[] = $node;
    ...
    public function consumers(array $leaves, Node $host, ErrorHandler $handler): array
    {
        ...
        foreach ($leaves as $leaf) {
            ...
            if ($transaction !== null) {
                $members[$transaction] = ($members->contains($transaction) ? $members[$transaction] : 0) + 1;
            }
        }
        ...
        foreach ($groups as $group) {
            ...
            foreach ($group as $leaf) {
                ...
                if ($transaction !== null) {
                    $inGroup[$transaction] = ($inGroup->contains($transaction) ? $inGroup[$transaction] : 0) + 1;
                }
            }
        ...
        foreach ($groups as $group) {
            ...
            foreach ($group as $leaf) {
                if ($leaf['transaction'] !== null && $opensHere->contains($leaf['transaction'])) {
    ...
    public function consumer(array $group, Node $host, ErrorHandler $handler): Loader
    {
        if (count($group) === 1) {
            ...
            return (
                $leaf['path'] === []
                    ? $leaf['loader']
                    : $this->feed->of($leaf['path'], [$leaf['loader']], $host, $handler, $this->next++)
            );
        }
        ...
        return $this->feed->of(
            $shared,
            $this->consumers($rest, $shared[array_key_last($shared)], $handler),
            $host,
            $handler,
            $sharedId,
        );
