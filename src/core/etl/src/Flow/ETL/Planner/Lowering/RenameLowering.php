<?php

declare(strict_types=1);

namespace Flow\ETL\Planner\Lowering;

use Flow\ETL\FlowContext;
use Flow\ETL\Loader;
use Flow\ETL\Plan\Node;
use Flow\ETL\Plan\Node\Rename;
use Flow\ETL\Planner\Lowering;
use Flow\ETL\Processor;
use Flow\ETL\Transformer;
use Flow\ETL\Transformer\RenameEntryTransformer;

/**
 * @implements Lowering<Rename>
 */
final readonly class RenameLowering implements Lowering
{
    /**
     * @return class-string<Rename>
     */
    public function handles(): string
    {
        return Rename::class;
    }

    /**
     * @param Rename $node
     *
     * @return list<Loader|Processor|Transformer>
     */
    public function steps(Node $node, FlowContext $context, array $frames): array
    {
        return [new RenameEntryTransformer($node->from, $node->to)];
    }
}
