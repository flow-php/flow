...
final class Mod extends ScalarFunctionChain
{
    ...
    public function eval(Row $row, FlowContext $context): ?int
    {
        ...
        if ($leftValue === null || $rightValue === null) {
            return $context
                ->functions()
                ->invalidResult(new InvalidArgumentException('Mod function requires non-null values'));
        }

        if ($rightValue === 0) {
            return $context
                ->functions()
                ->invalidResult(new InvalidArgumentException('Mod function cannot perform modulo by zero'));
        }
