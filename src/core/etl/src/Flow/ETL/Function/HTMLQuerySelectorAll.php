...
final class HTMLQuerySelectorAll extends ScalarFunctionChain
{
    ...
    public function eval(Row $row, FlowContext $context): ?array
    {
        ...
        if (null === $value) {
            return $context
                ->functions()
                ->invalidResult(new InvalidArgumentException('HTMLQuerySelectorAll requires non-null HTMLDocument'));
        }

        if (null === $selector) {
            return $context
                ->functions()
                ->invalidResult(new InvalidArgumentException('HTMLQuerySelectorAll requires non-null selector'));
        }
