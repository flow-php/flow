...
final class RoundTest extends FlowTestCase
{
    ...
    {
        static::assertEquals(10.12, ref('float')->round(lit(2))->eval(row(['float' => 10.123]), flow_context()));

        static::assertIsFloat(ref('float')->round(lit(2))->eval(row(['float' => 10.123]), flow_context()));
    }

    public function test_round_with_precision_0(): void
    {
        static::assertSame(10.0, ref('float')->round(lit(0))->eval(row(['float' => 10.123]), flow_context()));
    }

    public function test_constructor_default_matches_the_dsl_default(): void
    {
        $row = row(['float' => 10.12345]);
