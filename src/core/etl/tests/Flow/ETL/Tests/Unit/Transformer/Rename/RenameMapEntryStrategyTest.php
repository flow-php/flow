...
use function Flow\ETL\DSL\{int_entry, rename_map, row, str_entry};
...
final class RenameMapEntryStrategyTest extends FlowTestCase
{
    public function test_rename_filters_only_existing_names() : void
    {
        ...
        \sort($entryNames);
        self::assertSame(['new_a', 'new_b', 'other_column'], $entryNames);
    }
    ...
    public function test_rename_with_empty_renames() : void
    {
        $strategy = rename_map([]);
        $row = row(
            str_entry('column_a', 'a'),
            int_entry('column_b', 1),
        );
