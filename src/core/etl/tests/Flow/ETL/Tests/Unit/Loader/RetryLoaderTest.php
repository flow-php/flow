...
use function Flow\ETL\DSL\{config, delay_fixed, duration_milliseconds, flow_context, int_entry, retry_any_throwable, row, rows, write_with_retries};
...
final class RetryLoaderTest extends TestCase
{
    public function test_exhausting_all_retries() : void
    {
        ...
        $mockLoader->expects(self::exactly(4)) // 1 initial + 3 retries
            ->method('load')
            ->with($rows, $context)
    ...
    public function test_retry_on_transient_failure_that_succeeds() : void
    {
        ...
        $callCount = 0;
        $mockLoader->expects(self::exactly(2))
            ->method('load')
            ->with($rows, $context)
