...
final class OutputsTest extends FlowTestCase
{
    public function test_the_result_comes_first_then_every_sink_in_order(): void
    {
        ...
        static::assertSame(
            ...
            (new Outputs($result, new Sinks($write, $transaction)))->children(),
        );
    }

    public function test_sinks_are_the_collection_it_was_built_with(): void
    {
        $read = NodeMother::read();
        $sinks = new Sinks(new Write($read, to_memory(new ArrayMemory())));

        static::assertSame($sinks, (new Outputs(new Result($read), $sinks))->sinks());
    }
    ...
    public function test_it_needs_at_least_one_sink_beside_the_result(): void
    {
        ...
        new Outputs(new Result(NodeMother::read()), new Sinks());
    }

    public function test_with_children_returns_the_same_instance_when_children_are_identical(): void
    {
        $read = NodeMother::read();
        $node = new Outputs(new Result($read), new Sinks(new Write($read, to_memory(new ArrayMemory()))));
    ...
    public function test_with_children_refuses_a_first_child_that_is_not_a_result(): void
    {
        ...
        $this->expectException(InvalidLogicException::class);
        $this->expectExceptionMessage('The first child of Outputs must stay a Result, ' . $read::class . ' given');
