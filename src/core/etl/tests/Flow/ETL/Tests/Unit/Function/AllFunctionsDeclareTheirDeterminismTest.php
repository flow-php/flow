...
final class AllFunctionsDeclareTheirDeterminismTest extends FlowTestCase
{
    ...
    public function test_every_scalar_function_is_deterministic_except_the_declared_generators(string $class): void
    {
        ...
        // the fixtures build Uuid through uuid4() and Ulid without a ref - both generate; a user callable is never
        // trusted, and ToDateTime's fixture parses 'Y-m-d', leaving the time of day to the clock
        static::assertSame(
            !in_array(
                $class,
                [
