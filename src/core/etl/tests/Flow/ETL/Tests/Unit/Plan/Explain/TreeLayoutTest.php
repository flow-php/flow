...
final class TreeLayoutTest extends FlowTestCase
{
            ...
            └─ #3 Limit  reducing · transparent · streaming
               ...
               └─ #2 Select  preserving · transparent · streaming
                  ...
                  └─ #1 Read  source · transparent · streaming
                        Extractor: ArrayExtractor
            PLAN, (new TreeLayout())->render((new Outline(declarations: true))->of($plan->root)));
    }

    public function test_with_declarations_a_subtree_two_consumers_share_is_printed_once(): void
    {
        $filter = new Filter(NodeMother::read(), ref('id')->isNotNull());
        $plan = new LogicalPlan(new Outputs(new Result($filter), new Write($filter, to_memory(new ArrayMemory()))));
    ...
    public function test_with_declarations_a_joined_frame_is_part_of_the_tree(): void
    {
        ...
        static::assertSame(<<<'PLAN'
            #4 Result  preserving · transparent · streaming
            │  Rows this plan hands out: to the trigger, or to the node reading it
            └─ #3 CrossJoin  expanding · opaque · streaming · redefines unknown
               │  Type: cross
               ├─ #1 Read  source · transparent · streaming
               │     Extractor: ArrayExtractor
    ...
    public function test_with_declarations_a_pushed_limit_shows_on_the_read_line(): void
    {
        ...
        static::assertStringContainsString(
            ...
            (new TreeLayout())->render((new Outline(declarations: true))->of($plan->root)),
        );
    }

    public function test_with_declarations_a_run_trigger_keeps_a_result_beside_the_write(): void
    {
        $read = NodeMother::read();
        ...
        static::assertSame(<<<'PLAN'
            ...
            └─ #4 Write  preserving · opaque · streaming
               │  Loader: MemoryLoader
               └─ #1 Read (shared)
            PLAN, (new TreeLayout())->render((new Outline(declarations: true))->of(Trigger::run->plan(
            $select,
