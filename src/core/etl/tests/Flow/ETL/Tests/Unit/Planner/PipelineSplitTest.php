...
final class PipelineSplitTest extends FlowTestCase
{
    ...
    public function test_a_refused_plan_uses_raw_steps_for_every_node(): void
    {
        ...
        static::assertSame($planned->of($root)->steps[0], $plan->root()->segments()->steps()[2]);
        static::assertSame($planned->of($joinEach)->steps[0], $plan->root()->segments()->steps()[0]);
    }
    ...
    public function test_a_refusal_inside_a_sink_keeps_the_schema_of_the_returned_rows(): void
    {
        ...
        $logical = LogicalPlan::of($read, new Sinks(new Write($joinEach, to_memory(new ArrayMemory()))));
        $planned = new PlannedNodes();
        (new Planner())->node($logical->root, NodeMother::context(), $planned);
    ...
    public function test_a_bound_plan_uses_bound_steps_for_every_node(): void
    {
        ...
        static::assertSame($planned->of($root)->bound[0], $plan->root()->segments()->steps()[0]);
        static::assertNotSame($planned->of($root)->steps[0], $plan->root()->segments()->steps()[0]);
    }
    ...
    public function test_a_bare_sink_at_the_spine_root_is_a_step_of_the_spine(): void
    {
        ...
        $plan = (new Planner())->plan(
            new LogicalPlan(new Outputs(new Result($read), new Sinks(new Write($read, $loader)))),
            NodeMother::context(),
        );
            ...
            ->plan(
                new LogicalPlan(
                    new Outputs(
                        ...
                        new Sinks(new Write($batch, to_memory(new ArrayMemory()))),
                    ),
                ),
                NodeMother::context(),
            )
        ...
        $plan = (new Planner())->plan(
            new LogicalPlan(
                new Outputs(
                    ...
                    new Sinks(new Write($collect, to_memory(new ArrayMemory()))),
                ),
            ),
            NodeMother::context(),
        );
    ...
    public function test_a_sink_with_its_own_steps_is_fed_through_one_side_pipeline(): void
    {
        ...
        $plan = (new Planner())->plan(
            new LogicalPlan(new Outputs(new Result($read), new Sinks(new Write(NodeMother::select($read), $loader)))),
            NodeMother::context(),
        );
    ...
    public function test_a_node_two_sinks_share_off_the_spine_runs_once_in_one_side_pipeline(): void
    {
        ...
        $plan = (new Planner())->plan(
            new LogicalPlan(
                new Outputs(
                    new Result($read),
                    new Sinks(new Write($shared, to_memory($first)), new Write($shared, to_memory($second))),
                ),
            ),
            NodeMother::context(),
        );
    ...
    public function test_two_sinks_at_one_node_are_two_steps(): void
    {
        ...
        $plan = (new Planner())->plan(
            new LogicalPlan(
                new Outputs(new Result($read), new Sinks(new Write($read, $first), new Write($read, $second))),
            ),
            NodeMother::context(),
        );
    ...
    public function test_a_write_that_does_not_translate_to_a_loader_is_refused(): void
    {
        ...
        $logical = new LogicalPlan(new Outputs(new Result($read), new Sinks($write)));
        $planned = new PlannedNodes();
        (new Planner())->node($logical->root, NodeMother::context(), $planned);
