...
final class WithColumnTest extends FlowTestCase
{
    ...
    public function test_a_bare_reference_under_a_plain_name_is_an_alias(): void
    {
        $node = new WithColumn(NodeMother::read(), 'copy', ref('id'));

        static::assertEquals(Redefined::alias('copy', 'id'), $node->redefines());
    }

    public function test_a_bare_reference_under_a_definition_is_not_an_alias(): void
    {
        $node = new WithColumn(NodeMother::read(), int_schema('copy'), ref('id'));
