...
{
    public function test_with_root_returns_a_new_plan(): void
    {
        $plan = NodeMother::plan(NodeMother::read());
        $root = NodeMother::select(NodeMother::read());

        $replaced = $plan->withRoot($root);

        static::assertNotSame($plan, $replaced);
        static::assertSame($root, $replaced->root);
    }

    public function test_sink_roots_are_the_sink_multiple_children_after_the_result(): void
    {
        ...
        static::assertSame(
            [$first, $second],
            (new LogicalPlan(new SinkMultiple(new Result($read), $first, $second)))->sinkRoots(),
        );
    }

    public function test_sink_roots_is_empty_for_a_result_root(): void
    {
        static::assertSame([], (new LogicalPlan(new Result(NodeMother::read())))->sinkRoots());
    }

    public function test_consumers_are_the_node_under_the_result_and_under_every_write(): void
    {
        $read = NodeMother::read();
    ...
    public function test_transform_up_rewrites_a_node_shared_by_the_spine_and_a_sink_once(): void
    {
        ...
        static::assertSame($spineSelect, $rewritten->sinkRoots()[0]->children()[0]);
    }

    public function test_transform_up_refuses_a_sink_root_rewritten_to_a_non_sink(): void
    {
        $read = NodeMother::read();
        $plan = new LogicalPlan(new SinkMultiple(new Result($read), new Write($read, to_memory(new ArrayMemory()))));
