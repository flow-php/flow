...
final class PipelineExtractorTest extends FlowTestCase
{
    ...
    {
        $pipeline = new Pipeline(from_rows(rows(row(int_entry('id', 1)), row(int_entry('id', 2))), rows(row(int_entry('id', 3)), row(int_entry('id', 4))), rows(row(int_entry('id', 5)), row(int_entry('id', 6)))));
