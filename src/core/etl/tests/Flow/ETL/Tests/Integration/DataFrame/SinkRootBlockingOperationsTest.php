...
/**
 * A Transformation given to to_transformation() becomes a sink root: one sink pipeline per run, fed batch by batch
...
final class SinkRootBlockingOperationsTest extends FlowIntegrationTestCase
{
    ...
    public function test_drop_duplicates_inside_a_transformation_deduplicates_globally(): void
    {
        ...
        // hashes, and the sink pipeline keeps it alive for the whole run.
        $spy = new SpyLoader();
    ...
    {
        // The Transformation is expanded once, when write() builds the plan, so one sink pipeline serves the run.
        $expansions = 0;
        $spy = new SpyLoader();
